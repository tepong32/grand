...
- Voucher and Disbursement Workbench shadow vertical slice with one shared Budget–Accounting–Treasury case, governed supplier/payee selectors, authoritative F4.2 obligation/payable intake, pilot DV numbering, wet-signature rounds, JEV references, multi-check controls, bank advice, release, correction history, and checksum-backed outputs. Legacy cases retain their pilot OBR history; new F5.1 cases pin the authoritative obligation UUID/checksum and do not post a second Budget balance.
...
The current Voucher Workbench remains in shadow comparison mode. F5.1 now connects a requesting-office payable to its authoritative obligation, but the parent F5 phase still lacks accepted transaction variants, relationship patterns, documentary rules, templates, and end-to-end replay evidence. The delivery order, acceptance gates, and current-position matrix are documented in the [GRAND Finance complete-cycle roadmap](docs/FINANCE_ROADMAP.md).
