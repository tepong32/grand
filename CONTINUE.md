...
## Current v0.7.53 checkpoint - Returned DV signing-authority supersession

Branch: `codex/finance-returned-dv-print-supersession`, based on pushed `3025d37`. Governed DV returns to preparation or renewed signatures supersede obsolete signing jobs and outputs. An interrupted pending amendment receives an explicit superseded status; its original evidence and previously completed amendments remain intact. The reasoned return event retains affected print, output and amendment identities. Corrected work uses its new signature round and fresh review. All 80 focused voucher/signature tests passed in 43.644 seconds. All 674 project tests passed in 304.236 seconds. System, migration-drift, compilation and diff checks passed. FIN-GAP-026 verified; D-043 records the explicit supersession choice. Migration vouchers.0021 adds a status choice only; tested in isolated databases, not applied to live data. Next: reduce My Work coverage/technical-detail clutter observed at 390px, then detailed DV print/packet/amendment personal history and posting/synchronization/reopen coverage. Preserve the user database.
