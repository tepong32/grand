...
## Current v0.7.57 checkpoint - Stored authority for DV artifacts

Branch: `codex/finance-dv-artifact-owner-authority`, based on pushed `b5f9c53`. DV output generation and legacy TracePoint linking now require the stored pinned Finance owner before new actions and replay. Packet linking reloads the item/current packet before visibility and existing-association checks. Source-page controls share the same owner predicate. Accounting retains its existing artifact workflows after custody moves, while unrelated-office grants and combined-grant UAT accounts cannot mutate evidence. All 90 focused voucher/custody tests passed in 60.994 seconds. All 684 full-project tests passed in 407.127 seconds. System, migration-drift, compilation and diff checks passed. FIN-GAP-030 verified; D-047 documents the ownership decision. Reproduction: three tests produced seven failed assertions in 4.682 seconds, demonstrating unauthorized output/link/replay and forged packet visibility. No schema changes or live migrations. Next: FIN-GAP-029 remaining source-page controls, followed by detailed DV print/packet/amendment and posting/reopen history. Preserve owning Accounting artifact authority across custody and source maker-checker rules. User database hash unchanged. Disposable loopback browser server and both named QA sessions have been closed; ignored isolated fixture data remains available through .tmp/finance_browser.py for later QA.
