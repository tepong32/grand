...
## Current v0.7.60 checkpoint - Payment form readiness

Branch: `codex/finance-payment-form-readiness`, based on pushed and remotely verified `a2952a2`. Payment forms now use current-office source actions, eligible cancellation states and nonterminal custody-link controls. Returned checks no longer reduce the replacement default, and stale/returned instruments are excluded from release choices with recovery guidance. Initial bank-advice preparation is hidden once issued checks belong to a current batch. Generic signature-stage attention now uses the real print/signature selectors. PASS: 177 voucher/custody/My Work/operations tests in 96.163 seconds. PASS: 692 full-project tests in 248.224 seconds. System, migration-drift, compilation and diff checks passed. Five reproduced tests produced ten failed assertions across two runs (8.231 and 5.681 seconds). FIN-GAP-029 known findings are verified across v0.7.58-60; D-050 records this phase. Next: detailed DV print/packet/amendment personal history, then posting/reopen history and the remaining coverage table. Preserve immutable child identities, attribution and current source access. An initial focused run exposed a test-fixture routing interaction with newly granted Accounting issue permission; removing that test grant before actual issuance retained the intended Treasury route. Only final completed runs count. No source mutation-service/schema change or live migration; user database unchanged. Production remains NO-GO pending functional completion and actual acceptance gates.
