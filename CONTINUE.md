...
## Current v0.7.59 checkpoint - Source dependencies for print actions

Branch: `codex/finance-print-action-dependencies`, based on pushed and remotely verified `3ad30ed`. Controlled signing-copy queues and forms now require their existing DV preparation grant and pinned owner. Print recording and packet assembly use separate source selectors; only new packets need TracePoint preparation authority. Reasoned replacement printing remains available without being counted as the pending signature action. Missing-owner cases remain readable with amendment controls unavailable. PASS: 112 voucher/custody tests in 49.996 seconds. PASS: 82 My Work/operations tests in 49.149 seconds. System, migration-drift, compilation and diff checks passed. Reproduction: two tests, six failed assertions and one missing-owner page error in 3.321 seconds. D-049 records the choice. FIN-GAP-029 remains partially repaired: next address remaining payment controls and generic awaiting-signature workbench attention, then detailed DV print/packet/amendment and posting/reopen history. No source mutation-service or schema changes; no live migration. Full suite NOT RUN for this bounded read-selector/form change; affected voucher/custody and My Work/operations regressions were used, with v0.7.58 retaining the 689-test full baseline. User database unchanged.
