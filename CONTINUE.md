...
## Current v0.7.50 checkpoint - scheduled-run and qualification-evidence handoffs

Branch: `codex/finance-field-control-handoffs`, based on pushed `bc5b209`. Scheduled reconciliation runs and qualification evidence now have shared source actions, personal submitted Waiting and attributed history. Qualification custody resolves through the controlling plan cycle, while the observed cycle remains explicit. Run history distinguishes reconciliation from review with open exceptions, and the source history panel shows before/after reference corrections. All 48 focused field tests passed in 41.326 seconds; all 127 field/My Work/operations dependency tests passed in 94.146 seconds. System, migration-drift, compilation and diff checks passed. Full project suite NOT RUN - OUT OF SCOPE for read-only selectors/projections and source-history rendering; existing source decisions remain unchanged. Prior v0.7.49 full gate passed 662 tests. D-040 records scope and review attribution. Next: reproduce FIN-GAP-024 DV amendment authority while preserving owning-Accounting amendments after Treasury handoff, then print/packet/amendment and detailed posting/reopen history. Preserve the user database.
