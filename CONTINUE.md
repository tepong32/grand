...
## Current v0.7.56 checkpoint - Source-state voucher return routes

Branch: `codex/finance-voucher-return-route-parity`, based on pushed `dfbd600`. Voucher return forms and the locked return service now share the existing source-stage destinations and retained DV/payment/posted-JEV/draft blockers. The source page restricts the control to the current office and excludes combined-grant UAT users. Posted financial evidence still requires its governed adjustment route; a discarded draft restores the existing return route. All 84 focused voucher/custody tests passed in 46.247 seconds. All 678 full-project tests passed in 334.918 seconds. System, migration-drift, compilation and diff checks passed. FIN-GAP-028 verified; D-046 retains the decision. Isolated browser inspection confirms only Accounting preparation and Wet signatures at Accounting validation. Next: high-priority FIN-GAP-030 stored owner/item authority for DV outputs and legacy packet linking, then FIN-GAP-029 remaining source-page controls, followed by detailed DV print/packet/amendment and posting/reopen history. Preserve owning Accounting artifact work across custody; do not replace it with a blanket current-office restriction. Preserve the user database. Disposable browser fixture remains .tmp/finance_browser.py on loopback 8766 with isolated two-store data and Playwright sessions finance-qa/finance-review-qa; development fixtures only.
