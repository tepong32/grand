...
Next: continue M01 net-assets/cash-flow source classifications and statements; reproduce the noted contra-account presentation question before relying on the existing aggregation. Then M02 earlier-payable linkage. Do not divert into unrelated policy/concurrency work or further UX. Real forms, ordinary office replay and LGU acceptance remain open. Native test/server processes have ended; eGAPS and the user database remain untouched. Preserve `accounting/test_policy_concurrency.py` and the existing `db.sqlite3` change outside this checkpoint.
