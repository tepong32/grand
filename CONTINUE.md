...
## Current v0.7.42 checkpoint - operational field-assignment authority

Branch: `codex/finance-field-assignment-authority`, based on pushed `deca4d7`. Named exercise owners/witnesses, defect resolution owners and stakeholder reviewers now require authenticated active non-UAT identity at stored service boundaries. Source controls and task selection share the rule. Named cross-office read access and the authorized owning-office manager alternative for defect correction remain supported. All 52 focused field/My Work tests passed in 25.593 seconds; all 644 project tests passed in 196.190 seconds. System, migration-drift, compilation and diff checks passed. FIN-GAP-020 is verified; D-032 records the choice. Next: personal field-cycle, exercise and defect handoffs with source-linked audit history, then remaining nested controls/local forms and detailed history. Preserve the user database.
