...
The [F0.2/F0.3 Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md) supplies the earlier discovery dependency. Cutover readiness requires current recorded LGU-confirmed evidence with acceptance examples for all eight detailed coverage areas, a separate whole-scope decision matching the complete enabled-scope text, and no current linked blocker. A generated starter or recorded unresolved question remains valid preparation/discovery evidence but cannot silently pass its named scope.
