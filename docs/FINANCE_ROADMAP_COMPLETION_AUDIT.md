...
- `python manage.py test finance.test_setup_discovery_register finance.test_discovery finance.test_operations departments.tests.InternalHowToTests --verbosity 1` — all 33 selected setup/discovery, Finance-entry/work-attention, export, and guide tests passed; and
- `python manage.py test --verbosity 1` — all 443 project tests passed across the default and separately routed Finance databases in 106.408 seconds of test execution.

An authenticated Playwright browser pass also exercised the live two-store setup, discovery, and My Work routes at desktop and 390×844 layouts, matching exact setup/discovery rows and counts, permission-shaped choices, and the open non-modal Accounting/setup guide with zero application-route console errors. This is visual software evidence, not named-user accessibility or field acceptance.
...
| F1 architecture, identity, and audit | One permission-shaped Finance entry, permission-filtered shared voucher-to-release case finder, private current-scope-rechecked saved views, exact-drill work-attention foundation for supported setup/discovery/Budget/Voucher/Accounting/bank-advice/returned-payment/period-close/cash-policy/cash-position/remittance/Reporting queues, department guides, role-shaped domain workspaces, department/object boundaries, immutable audit patterns, cross-cycle lineage, and two-store routing | Field/local-form and remaining cross-cycle source adapters; stable item-level Task ID/type, action/gate, owner/queue, timing, version and exception contract; Waiting/Returned/Due/Completed-by-me; named-user role validation; governed shared-view/search/following/notification acceptance; actual segregation-of-duties approval; local security/privacy review; and supported-device/accessibility observation |
