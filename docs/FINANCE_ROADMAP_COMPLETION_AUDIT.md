...
- `python manage.py test finance.test_cutover finance.test_operations departments.tests.InternalHowToTests --verbosity 1` — all 45 selected field-operation, Finance-entry/work-attention, export, and guide tests passed; and
- `python manage.py test --verbosity 1` — all 449 project tests passed across the default and separately routed Finance databases in 103.170 seconds of test execution.

An authenticated Playwright browser pass also exercised the isolated live two-store field-operation and My Work routes at desktop and 390×844 layouts. It matched the exact one-cycle source-lock queue and export scope to a My Work count of one, kept the prepared-cycle count separately at one, displayed permission-shaped choices and plain next-action guidance, and kept the non-modal Accounting guide open with zero application-route console errors. This is visual software evidence, not named-user accessibility or field acceptance.
...
| F1 architecture, identity, and audit | One permission-shaped Finance entry, permission-filtered shared voucher-to-release case finder, private current-scope-rechecked saved views, exact-drill work-attention foundation for supported setup/discovery/field-operation/Budget/Voucher/Accounting/bank-advice/returned-payment/period-close/cash-policy/cash-position/remittance/Reporting queues, department guides, role-shaped domain workspaces, department/object boundaries, immutable audit patterns, cross-cycle lineage, and two-store routing | Local-form and remaining cross-cycle source adapters; stable item-level Task ID/type, action/gate, owner/queue, timing, version and exception contract; Waiting/Returned/Due/Completed-by-me; named-user role validation; governed shared-view/search/following/notification acceptance; actual segregation-of-duties approval; local security/privacy review; and supported-device/accessibility observation |
