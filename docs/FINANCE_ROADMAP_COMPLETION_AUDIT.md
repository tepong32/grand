...
- `python manage.py test vouchers.test_cash_register vouchers.test_advice_register accounting.test_period_close finance.test_operations departments.tests.InternalHowToTests --verbosity 1` — all 28 selected source-adapter, Finance-entry/work-attention, and guide tests passed; and
- `python manage.py test --verbosity 1` — all 436 project tests passed across the default and separately routed Finance databases in 83.634 seconds of test execution.

An authenticated Playwright browser pass also exercised the live two-store cash-position route at desktop and 390×844 layouts, the exact returned-position work row and source link, and the open non-modal Treasury guide with zero console errors. This is visual software evidence, not named-user accessibility or field acceptance.
...
| F1 architecture, identity, and audit | One permission-shaped Finance entry, permission-filtered shared voucher-to-release case finder, private current-scope-rechecked saved views, exact-drill work-attention foundation for supported Budget/Voucher/Accounting/bank-advice/period-close/cash-policy/cash-position/remittance/Reporting queues, department guides, role-shaped domain workspaces, department/object boundaries, immutable audit patterns, cross-cycle lineage, and two-store routing | Returned-item, field/setup/discovery/local-form and remaining source adapters; stable item-level Task ID/type, action/gate, owner/queue, timing, version and exception contract; Waiting/Returned/Due/Completed-by-me; named-user role validation; governed shared-view/search/following/notification acceptance; actual segregation-of-duties approval; local security/privacy review; and supported-device/accessibility observation |
