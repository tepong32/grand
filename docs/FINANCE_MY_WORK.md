...
The current source-adapter queue for personal Waiting and retained completion is complete for the domains in [the coverage table](FINANCE_WORK_VIEW_COVERAGE.md), including source staging, custody, posting and field-owner handoffs through v0.7.68. Ready, Returned, retained-date views and the separately administrator-assigned bounded portable export remain source-scoped. The broader F1 contract still defers generic assignment/following and governed shared views until their ownership/privacy rules are defined. Notifications must remain signals over the same source contract, never a parallel queue. These deferred features do not establish Finance production authority; mandatory operational scrutiny and LGU acceptance remain required.
...
## v0.7.41 - accountability profile/package work

My Work now includes accountability profile/package preparation, returned correction and independent review under shared current-office source selectors. Own submitted versions appear in Waiting; retained submission, return, profile activation and package approval events provide personal history. Existing cross-office evidence recipes remain supported, and checksum exceptions explain why approval is blocked while a return remains available. All 13 focused accountability tests passed in 7.533 seconds; all 188 My Work/reporting dependency tests passed in 66.194 seconds. System, migration-drift, compilation and diff checks passed. Draft/evidence-selection details and indirect predecessor supersession remain in source audit; completion credits explicit lifecycle actions. No period-derived deadlines are invented. D-031 records the source parity decision.


## v0.7.43 - field cycle, exercise and defect handoffs

Waiting follows own cycle reconciliation submissions, submitted exercise results and submitted defect corrections, preserving current cycle read scope and distinct child identities. Completion resolves explicit audit events through matching target cycle, department and child ID. Submission and independent acceptance remain separate actions; current status does not rewrite who performed a retained step. Named cross-office access remains intact, and assignment revocation is rechecked. No new review deadlines are inferred from original exercise/correction targets. Other field plans/runs, stakeholder/cutover history and returned-cycle successor actions remain separate work. All 36 focused field-workflow tests passed in 13.056 seconds; all 224 field/My Work/reporting dependency tests passed in 80.786 seconds. System, migration-drift, compilation and diff checks passed. D-033 records the choice.
...
Waiting now follows exercises scheduled and defects registered by the user when prepared work is with another named owner. Creator attribution continues through submitted-result review. Current actions are excluded before the display cap. Returned correction uses only the latest matching return after the current submission; invalid latest evidence leaves age unknown rather than reusing an older date. Existing source assignment and review permissions remain authoritative; this is personal tracking, not generic delegation.
