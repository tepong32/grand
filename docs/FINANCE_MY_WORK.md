...
Status: F1.5 foundation plus the first two item-level task-contract checkpoints implemented. `/finance/my-work/` provides a live, permission-filtered overview of supported Finance setup/discovery/field operation, Budget, shared voucher, Accounting, bank-advice, returned-payment, Treasury remittance, cash-policy, cash-position, period-close/reopen, Reporting, and local-form action groups. Field-operation cycle gates, named nested field actions, and local-form actions also expose stable read-only work items over their existing governed records; the remaining domains are not yet represented at item level.
...
For Field-operation cycle gates, named defects, readiness exercises, stakeholder/cutover decisions, and Local forms, each visible item now carries:
