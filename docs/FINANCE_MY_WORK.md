...
Status: F1.5 foundation implemented. `/finance/my-work/` provides a live, permission-filtered overview of supported Budget, shared voucher, Accounting, Treasury remittance, and Reporting action groups. It is a read model over existing governed registers, not yet the complete item-level My Work task contract.
...
- Accounting JEV draft/posting, opening-balance preparation/submission/review/post/reconcile, and bank-statement staging/correction/matching/review;
...
- Accounting users learn JEV/opening/bank/reporting handoffs; and
- Treasury users learn voucher/remittance triage and why unadapted bank-advice/cash-position counts are not guessed.
...
Automated coverage verifies the stable route and Finance-entry link, denial for reporting-only/ordinary users, exact requesting-office voucher count parity with the filtered source queue, hidden-office isolation, UAT preview exclusion, boundary copy, and publication of the page-relevant department guide. The full Finance and project suites remain the release regression gate.
