...
A governed return to DV preparation or renewed signatures supersedes old signing copies and outputs before a fresh round proceeds. Returning to preparation also marks any interrupted pending amendment **Superseded by returned DV correction**. It does not mark that amendment completed or rewrite its original financial/date/signatory evidence. The retained return event records affected print versions, output IDs and amendment versions with the actor, server time and correction reason. Previously completed amendments and signed-task evidence remain unchanged. A corrected DV requires fresh printing, packet confirmation and signatures; the interrupted amendment does not authorize a later-stage shortcut. See D-043 in [the decision log](IMPLEMENTATION_DECISIONS.md).

The reasoned return form and locked return service share the stage destinations and retained-source blockers in `vouchers.return_routes`. The source page limits its return control to the current office and displays posted-JEV or materialized-draft restrictions explicitly. The service still rechecks the stored case, actor, current office, reason and version; an altered form or caller object cannot widen the route. The existing discard workflow must resolve a draft JEV before a blocked return becomes available. Posted evidence still needs the governed adjustment/reversal/replacement path. See D-046 in [the decision log](IMPLEMENTATION_DECISIONS.md).

DV output generation and legacy TracePoint linking use the same pinned Finance-owner boundary as nonfinancial amendments, with their own explicit permissions and active/non-UAT checks. The services reload and lock the stored case before owner checks, including replay, and legacy linking reloads the item/current packet before visibility and association checks. The source page shares that owner predicate. Owning Accounting may still generate its DV output after Treasury handoff or link its visible legacy packet during Budget custody; unrelated-office grants do not delegate these actions. See D-047 in [the decision log](IMPLEMENTATION_DECISIONS.md).

Voucher preparation and independent payable review forms use their source action selectors. The wet-signature return form uses the actor's eligible source-task selector for both display and submission, including configured order, current office and controlled-packet readiness. The locked service still verifies the stored task and order. UAT mutation display is denied even with combined operational grants while separate audit/read permissions remain; a pending amendment suppresses another amendment form until its governed resolution. Printing dependencies and other payment controls remain tracked in FIN-GAP-029. See D-048 in [the decision log](IMPLEMENTATION_DECISIONS.md).


## v0.7.59 - Source dependencies for print actions

Controlled signing-copy queues and forms now require their existing DV preparation grant and pinned owner. Print recording and packet assembly use separate source selectors; only new packets need TracePoint preparation authority. Reasoned replacement printing remains available without being counted as the pending signature action. Missing-owner cases remain readable with amendment controls unavailable. See D-049 in [the decision log](IMPLEMENTATION_DECISIONS.md) and [the completion audit](FINANCE_ROADMAP_COMPLETION_AUDIT.md) for verification.
