...
Budget, Accounting, Treasury, and Finance configuration roles can open **Local form acceptance** from the reporting workspace. Preparers may enter the current office form manually or use the in-app catalog of 31 source-anchored DBM LBP/LBA/LBR/LBE/LBAc candidates. A catalog choice creates only an editable, source-unmapped, delivery-unconfirmed draft. Every candidate section must be compared with the current local blank/redacted form and resolved with an attributable matched or not-applicable reference. Preparers then link one exact activated report template or active preflighted Finance workbook and upload only a blank or safely redacted reference. Seven practical test categories cover data, layout, signatory/custody route, overflow, accessibility/download, printer/form stock, and rollback/recovery.
