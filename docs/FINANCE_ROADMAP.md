...
Implementation position: F1.2 adds one plain Finance entry page whose cards are composed only from existing Budget, Voucher, Accounting, Reporting, Finance setup/discovery, and field-operation access checks. F1.3 adds a controlled search handoff into the existing permission-filtered Voucher Workbench and shared voucher-to-release timeline; F1.4 adds user-named private case-filter views that are rechecked against current role/department scope and remain removable non-authoritative preferences. These controls do not create a second case index, disclose hidden-case counts or suggestions, assign work, or satisfy the broader pre-voucher My Work/governed-shared-view/notification contract. Reporting alone does not classify a general employee as Finance; a named cross-office field reviewer receives only the applicable field destination; existing direct links remain available; and each destination rechecks its own scope. A department-seeded floating guide explains shared-case handoffs, private views, governed corrections, exports, and private tutorial progress. Native MySQL logical backup creation produces one verified, atomically published, checksum-manifested recovery set for both the default and Finance stores, with guarded retention, a separately retainable manifest hash, strict read-only copied-set verification, and an operator restore runbook. A non-root Python 3.11/Gunicorn container, WhiteNoise static delivery, two-store environment contract, stdout logging, minimal health probe, and non-secret configuration/live production preflight provide the deployable runtime foundation. Actual named-user role walkthroughs, complete-cycle My Work/governed-shared-view/notification acceptance, Render topology, persistent/shared storage, backup publication schedule, off-host copying, encryption/retention decisions, monitoring, and a witnessed two-store restore rehearsal with reconciled control totals remain F1/cutover acceptance gates; a visible card, search result, saved preference, built image, preflight/integrity receipt, or created artifact is not represented as authority, production acceptance, or restore proof. See [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md).
...
|---|---|---|
| Finance operations entry | F1.2 permission-shaped landing page and guide, F1.3 permission-filtered shared voucher-to-release case finder, and F1.4 private current-scope-rechecked saved case views implemented over the existing domain workspaces | Perform named-user desktop/narrow-layout and accessibility review; complete broader pre-voucher/cross-cycle My Work, governed shared views, search, and notifications without duplicating domain queues or weakening access controls. |
...
2. **F1.1–F1.4 — Complete-cycle information architecture, permission-shaped entry, shared-case finder, and private saved views — implemented design and bounded controls**: the role/permission matrix, case/timeline/search/status contract, and synthetic prototype remain the design baseline; `/finance/` provides one plain starting page and a controlled-reference handoff into the existing permission-filtered Voucher Workbench, where users may keep removable private filter shortcuts that are rechecked against current scope. Named-user walkthroughs, broader pre-voucher/cross-cycle My Work, governed shared views, search, notifications, supported-device/accessibility observation, and parent-F1 acceptance remain open; see [complete-cycle Finance information architecture](finance-ia/README.md) and [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md).
...
F1.2–F1.4 delivery is recorded separately in [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md); it implements permission-shaped navigation, the existing shared voucher-to-release case finder, and private current-scope-rechecked saved views from the F1.1 contract while leaving broader pre-voucher/cross-cycle My Work, governed shared views, search, notifications, and field acceptance open.
