...
Implementation position: F11.1 adds limited shadow/parallel cycle plans, exact comparisons, owned blocking defects, independent reconciliation, named seven-party acceptance, exact-scope authority, rollback, guidance, and TraceSync-ready evidence. F11.2 adds versioned redacted CSV staging, automatic file/layout locks, safe heading/row-count inspection, predecessor/prior-version schema-drift detection, independent drift acceptance, retained replacements, and metadata-only portable evidence; see [Finance shadow operation, UAT acceptance, and controlled cutover](FINANCE_SHADOW_CUTOVER.md). A production/eGAPS adapter, scheduled reconciliation cadence, locally accepted escalation/curricula/runbooks, named non-functional exercises, consecutive field cycles, and actual stakeholder/cutover signatures remain open, so the parent F11 exit gate is not claimed.
...
| Shadow operation and cutover | F11.1 governed cycles/cutover plus F11.2 versioned redacted source staging, automatic locks, and independent schema-drift gate implemented | Confirm any production adapter and cadence, curricula/runbooks, non-functional exercises, consecutive local field cycles, and actual seven-party approval before claiming authority. |
