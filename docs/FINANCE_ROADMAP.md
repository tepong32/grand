...
Current v0.7.87 development checkpoint: [historical invoice allocations](FINANCE_HISTORICAL_SPLITS_IN_PROGRESS.md) now cover reviewed invoice rows within one original credit, native applications/reversals, invoice DV settlement and ordinary proposal entry. HTTP tests connect entry and independent approval to deductions, payments, returns/corrections and claim CSV. Synthetic browser checks cover proposal validation/approval and readable invoice entry at desktop/narrow widths. This checkpoint does not close M02 or establish full Finance parity. Shared applications across distinct original credits, generated-history adoption, later corrections and the preserved M01/M03, inventory, exact-output and operational/LGU gates remain open.
...
Current M02 integration: [prior-payable DV settlement](FINANCE_PRIOR_PAYABLE_DV_2026-09-10.md) connects named source claims to validation, withholding, actual payment, cancellation, returned instruments and replacement, with recoverable reservations across the separate stores. Read the validation record before relying on this checkpoint. Configured earlier-accrual generation, historical attribution and remaining financial corrections are still open. Earlier “DV linkage blocked” statements describe prior checkpoints.
...
Implementation position: F0.1 provides repository-safe interview, evidence, actual-step, role/signature, decision, and redacted-replay worksheets. F0.2 adds a plain-language in-app Decisions & evidence register with the five evidence labels, named cross-office owner/reviewer access, immutable submitted snapshots and checksums, recorded unresolved scope blockers, reasoned successors, an Accounting-specific floating guide, and audited per-record TraceSync CSV export. F0.3 adds nine editable, safely unresolved candidate-cycle prompts, retained acceptance examples, and visible 0–8 detailed-area coverage. F0.4 adds a filter-aware, department-bounded register CSV with archive receipt, audit event, and spreadsheet-formula protection. F0.5 adds role-bounded candidate-cycle and practical attention triage, overdue marking, and synchronized export filters without another workflow state. A linked cycle cannot pass cutover readiness without independently recorded LGU-confirmed examples for all eight areas, a separate whole-scope decision matching its exact enabled scope, and no current linked blocker; see [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md). Actual interviews, local sources, custody records, applicability decisions, and named-office acceptance remain open, so the parent F0 exit gate is not claimed.
...
Implementation position: F1.2 adds one plain Finance entry page whose cards are composed only from existing Budget, Voucher, Accounting, Reporting, Finance setup/discovery, and field-operation access checks. F1.3 adds a controlled search handoff into the existing permission-filtered Voucher Workbench and shared voucher-to-release timeline; F1.4 adds user-named private case-filter views that are rechecked against current role/department scope and remain removable non-authoritative preferences. F1.5 begins the My Work read model with defined, generated-at, permission-filtered action counts whose links reproduce the same Budget, shared-case, Accounting, local-form, bank-advice, returned-payment, period-close/reopen, cash-control, remittance, or Reporting scope and state; unsupported adapters are omitted and UAT preview stages are excluded from personal actions. Source adapters share exact filters between My Work and the setup, discovery, field-operation, local-form, bank-advice, returned-payment, period-close, and cash-control registers. Local forms separate preparation/returned correction, independent test witnessing, and independent acceptance review from accepted/superseded oversight. Returned payments are separated into Accounting decision, Treasury clarification, and Accounting-cleared controlled-replacement queues; cash policies and cash-position versions likewise remain distinct. These controls do not create a second case index, disclose hidden-case counts or suggestions, assign work, notify users, or yet satisfy the stable item-level pre-voucher/cross-cycle My Work and governed-shared-view contract. Reporting alone does not classify a general employee as Finance; a named cross-office field reviewer receives only the applicable field destination; existing direct links remain available; and each destination rechecks its own scope. Department-seeded floating guides explain shared-case handoffs, private views, attention triage, governed corrections, exports, and private tutorial progress. Native MySQL logical backup creation produces one verified, atomically published, checksum-manifested recovery set for both the default and Finance stores, with guarded retention, a separately retainable manifest hash, strict read-only copied-set verification, and an operator restore runbook. A non-root Python 3.11/Gunicorn container, WhiteNoise static delivery, two-store environment contract, stdout logging, minimal health probe, and non-secret configuration/live production preflight provide the deployable runtime foundation. Actual named-user role walkthroughs, complete-cycle My Work/governed-shared-view/notification acceptance, Render topology, persistent/shared storage, backup publication schedule, off-host copying, encryption/retention decisions, monitoring, and a witnessed two-store restore rehearsal with reconciled control totals remain F1/cutover acceptance gates; a visible card, count, search result, saved preference, built image, preflight/integrity receipt, or created artifact is not represented as authority, production acceptance, or restore proof. See [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md) and [Finance work-attention foundation](FINANCE_MY_WORK.md).

Current F1.5 checkpoint: setup releases use exact preparation, independent-review, future-scheduling, and due-activation adapters; discovery decisions use exact owner/manager preparation and named-independent-review adapters while blocker/overdue views remain oversight. Field operation adds permission-shaped cycle preparation/execution/reconciliation/authority plus named defect/exercise/witness/stakeholder actions, while reconciled/authorized/returned cycle views remain oversight. Local forms add separate preparation, returned-correction, independent-test-witness, and independent-acceptance actions while accepted/superseded views remain oversight. Budget proposals/allotments/obligations and payable-intake preparation/review now have stable exact items. Payable rows reuse the shared current-office and maker-checker filters, expose zero-difference and data-integrity blockers, and keep missing deadlines explicit rather than inferred. Source pages, filtered exports, and My Work reuse the same bounded queries. The stable item-level contract projects these records into deterministic source-linked Task IDs with action/gate, permitted queue, scope, timing basis, source state/version, exception, and authoritative-record link without writing parallel workflow state. DV/custody, Accounting, Treasury, and Reporting item adapters plus remaining cross-cycle capabilities remain open.
...
Implementation position: F8.1–F8.5 synthetic controls are implemented. F8.1 covers cross-voucher withholding remittance through posted liability reduction. F8.2 covers checksummed monthly bank-statement versions, exact/guided posted-GL matching, adjusted-balance timing items, independent zero-difference reconciliation, Internal How-Tos, and portable evidence. The monthly landing page now adds guided source/control/matching/review triage and a synchronized multi-statement TraceSync register while retaining each statement's detailed row/match/timing-item export. F8.3 adds locally versioned Observe/Enforce cash policies, independently approved reconciliation-backed positions, issue reservations, configurable unclaimed/stale/returned classification, department guidance, and portable evidence. F8.4 adds retained multi-case advice versions, independent review, actual bank submission and acknowledgement/return evidence, acknowledgement-gated release, reasoned successors, and returned-payment reversal/reissue orchestration. F8.5 adds prior-reconciled timing-item carry-forward, age/overdue visibility, later-bank clearance across retained lineage, and reason-required reopening; see [Finance remittance execution](FINANCE_REMITTANCE_EXECUTION.md), [Finance bank reconciliation](FINANCE_BANK_RECONCILIATION.md), [Finance cash position and instrument ageing](FINANCE_CASH_POSITION.md), and [Finance bank advice and returned instruments](FINANCE_BANK_ADVICE.md). Exact bank terms/channels/files, official forms/rules, local ageing/escalation acceptance, and full consecutive-month redacted instrument-to-bank replay remain acceptance/delivery gates.
...
Implementation position: F10.1 adds inactive human-editable versions, safe preview generation, automatic same-period golden data/control comparison, plain-language form/print checks, template diffs, schedule-impact review, independent approval, deployment-free activation, reasoned rollback, append-only events, department roles, inline guidance, and TraceSync-ready receipts. F10.2 adds a department-bounded plain-language form register, guided mapping/reference/section/test/review triage, a synchronized checksum/test-count-aware TraceSync summary export, an in-app catalog for all 31 source-anchored DBM LBP/LBA/LBR/LBE/LBAc candidates, 77 editable field/source/control/owner/print section groups with explicit local-resolution evidence, dynamic required/optional/conditional/repeating behavior, exact promoted-report or active-preflighted-workbook source locks, seven independently witnessed practical tests, maker–checker local acceptance, reasoned fully retested successors, schema-compatible historical reproducibility, guidance, and detailed TraceSync-ready evidence; see [Finance visual template promotion and rollback](FINANCE_TEMPLATE_PROMOTION.md) and [Finance local-form inventory and acceptance](FINANCE_LOCAL_FORM_ACCEPTANCE.md). Starters remain unmapped, delivery-unconfirmed drafts with no reference, tests, or acceptance. Actual LGU forms, accepted authorities/decisions, golden outputs, devices, physical printer/form-stock trials, retained samples, and named-office sign-off remain open, so the parent F10 exit gate is not yet claimed.
...
Implementation position: F11.1 adds limited shadow/parallel cycles, exact comparisons, independent reconciliation, seven-party acceptance, exact-scope authority, rollback, guidance, and portable evidence. F11.2 adds versioned redacted CSV staging, automatic file/layout locks, safe inspection, predecessor/prior-version drift detection, independent drift acceptance, and retained replacements. F11.3 adds maker–checker local cadence/severity/escalation plans, scheduled checksummed run snapshots, reviewed-exception visibility, due/overdue defect triage, attributable escalation, independent correction acceptance, and minimum-run/final-resolution gates. F11.4 adds a maker–checker curriculum/quick-guide/supervisor/support plan, one independently witnessed role exercise per stakeholder, eight named nonfunctional exercise categories, reasoned reruns, private tutorial-progress separation, and stricter cutover readiness gates. F11.5 adds a locally editable maker–checker field-qualification plan, independently accepted consecutive-cycle evidence, explicit predecessor/parallel-run gates, and retained stakeholder/cutover signed-record references and checksums. F11.6 binds that plan and every field-cycle submission to exact current F10.2 accepted-form snapshots and checksums, with supersession/drift blocking readiness. F11.7 requires the backup/restore exercise to bind an exact off-host two-store set, approved/actual RPO/RTO, both restored stores and migrations, reconciled controls, a cross-store case, runtime-file/disposal evidence, resolved exceptions, independent witness passage, and the same immutable evidence in the cutover decision. F11.8 adds a role-bounded Field Acceptance Board that groups these existing governed records into ten plain-language checkpoints, preserves the separate authority boundary, and exports an audited TraceSync-ready status index. F11.9 adds synchronized role-bounded cross-cycle triage and a spreadsheet-safe oversight register derived from that board and retained under the TraceSync-ready export root; see [Finance shadow operation, UAT acceptance, and controlled cutover](FINANCE_SHADOW_CUTOVER.md) and [Finance Field Acceptance Board](FINANCE_FIELD_ACCEPTANCE_BOARD.md). A production/eGAPS adapter and the actual locally accepted content, field execution, forms, recovery results, pass conditions, and signatures remain open, so the parent F11 exit gate is not claimed.

## Current implementation position

| Capability | Current position | Roadmap consequence |
|---|---|---|
| Finance operations entry | F1.2 permission-shaped landing page, F1.3 shared voucher-to-release case finder, F1.4 private current-scope-rechecked saved case views, F1.5 exact-drill permission-filtered attention overview with setup, discovery, field-operation, local-form, bank-advice, returned-payment, period-close/reopen, cash-policy, and cash-position source adapters, plus stable item projection for Finance Setup releases, discovery decisions, Budget proposal/allotment/obligation controls, payable-intake preparation/review, Field-operation cycle/nested-record gates, and Local forms | Extend the item contract to DV/custody, Accounting, Treasury, Reporting, and other supported domains; add remaining cross-cycle capabilities; perform named-user desktop/narrow-layout and accessibility review; and complete governed shared views, broader search, following, and notifications without duplicating domain queues or weakening access controls. |
| Governance and discovery | F0.1 field templates, F0.2 governed Decisions & evidence register, F0.3 editable cycle coverage/acceptance-example controls, F0.4 filtered department export, and F0.5 guided triage implemented | Populate and extend the starters from actual interviews/current sources, resolve every enabled-scope gap, and obtain independent LGU confirmation; filters, generated prompts, and portable indexes cannot satisfy cutover readiness. |
...
1. **F0.1–F0.5 — Finance evidence register, interview kit, governed decisions, editable coverage, portable register, and triage — implemented preparation control**: repository-safe field templates plus an in-app maker–checker register, nine non-authoritative candidate-cycle prompts, retained acceptance examples, exact-scope blockers, reasoned successors, visible coverage, cycle/attention work-list filters, cycle-linked cutover gating, floating guidance, and audited synchronized per-record/department TraceSync exports; see [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md). Actual field evidence and parent-F0 acceptance remain open.
2. **F1.1–F1.5 — Complete-cycle information architecture, permission-shaped entry, shared-case finder, private saved views, and work-attention foundation — implemented design and bounded controls**: the role/permission matrix, case/timeline/search/status contract, and synthetic prototype remain the design baseline; `/finance/` provides one plain starting page, controlled-reference case handoff, removable private filter shortcuts, exact-drill action counts over supported existing registers, and stable item projection for Finance Setup releases, discovery decisions, Budget proposal/allotment/obligation controls, payable-intake preparation/review, Field-operation cycle/nested-record gates, and Local forms. DV/custody, Accounting, Treasury, Reporting, and other-domain item adapters, remaining cross-cycle capabilities, named-user walkthroughs, governed shared views, broader search, following/notifications, supported-device/accessibility observation, and parent-F1 acceptance remain open; see [complete-cycle Finance information architecture](finance-ia/README.md), [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md), and [Finance work-attention foundation](FINANCE_MY_WORK.md).
3. **F2.1 — Fiscal-year and classification foundation — implemented synthetic control**: typed fiscal year, finance calendar, PPAs/MFOs, projects/activities, funding sources, expanded account/office dimensions, readiness layers, setup migration path, a shared office/fiscal-year transaction-store lock that serializes cross-store setup amendments with controlled voucher/check number issuance, and an audited TraceSync-ready selected-year/all-years review register with role-specific floating guidance; see [Finance fiscal-year and classification foundation](FINANCE_FISCAL_FOUNDATION.md). Actual classifications, authority, readiness evidence, calendars, and named-owner acceptance remain parent-F2 gates.
...
22. **F9.1 — Finance accountability reporting and reproducibility — implemented synthetic control**: cumulative Budget accountability and posted-trial-balance starters, candidate/locally-confirmed applicability controls, immutable data/control/source snapshots, control-gated review, permission-preserving guided run triage, a synchronized one-row-per-run TraceSync register, drill-through, deterministic reproduction receipts, and role guidance; see [Finance accountability reporting](FINANCE_ACCOUNTABILITY_REPORTING.md). Accepted official forms, signed-reference reproduction, complete local package evidence, and the parent-F9 exit gate remain open.
