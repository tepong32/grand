...
| Standalone Accounting | Fiscal/fund/account setup, controlled opening intake/posting/reconciliation/export, F7.1 rule-backed JEVs, F7.2 subsidiary controls, and F7.3 payment-event JEV orchestration implemented | Continue under F7–F8; execute remittances, broaden subsidiary ledgers, close periods, and produce statements. |
...
| Treasury | Check registration, cancellation/replacement, advice/release controls, F7.3 payment posting, explicit no-entry decisions, and portable case registers implemented | Complete printing, cash control, remittance execution, consolidated registers, unclaimed/stale handling, and bank reconciliation in F8. |
