...
|---|---|---|
| Finance operations entry | F1.2 permission-shaped landing page, F1.3 shared voucher-to-release case finder, F1.4 private current-scope-rechecked saved case views, and F1.5 exact-drill permission-filtered attention overview with setup, discovery, bank-advice, returned-payment, period-close/reopen, cash-policy, and cash-position source adapters implemented over existing domain workspaces | Add field/local-form and remaining cross-cycle source adapters plus the stable item-level task contract; perform named-user desktop/narrow-layout and accessibility review; complete governed shared views, broader search, following, and notifications without duplicating domain queues or weakening access controls. |
...
2. **F1.1–F1.5 — Complete-cycle information architecture, permission-shaped entry, shared-case finder, private saved views, and work-attention foundation — implemented design and bounded controls**: the role/permission matrix, case/timeline/search/status contract, and synthetic prototype remain the design baseline; `/finance/` provides one plain starting page, controlled-reference case handoff, removable private filter shortcuts, and exact-drill action counts over supported existing registers, including setup, discovery, bank advice, returned payment, period close/reopen, cash policy, and cash position. Remaining field/local-form/cross-cycle adapters, stable item-level cross-cycle tasks, named-user walkthroughs, governed shared views, broader search, following/notifications, supported-device/accessibility observation, and parent-F1 acceptance remain open; see [complete-cycle Finance information architecture](finance-ia/README.md), [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md), and [Finance work-attention foundation](FINANCE_MY_WORK.md).
...
F1.2–F1.5 delivery is recorded in [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md) and [Finance work-attention foundation](FINANCE_MY_WORK.md); it implements permission-shaped navigation, the existing shared voucher-to-release case finder, private current-scope-rechecked saved views, and exact-drill supported attention groups, including setup, discovery, bank advice, returned payment, period close/reopen, cash policy, and cash position, while leaving field/local-form/cross-cycle adapters, the stable item-level task contract, governed shared views, broader search, following/notifications, and field acceptance open.
