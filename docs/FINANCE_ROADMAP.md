...
Implementation position: F9.1 supplies implemented synthetic controls for a cumulative Budget accountability schedule and posted trial balance, with explicit candidate-versus-locally-confirmed applicability, immutable data/control/source evidence, control-gated review, source drill-through, reproduction receipts, editable native starters, department guidance, and TraceSync-ready exports; see [Finance accountability reporting](FINANCE_ACCOUNTABILITY_REPORTING.md). The full report catalog, financial statements/notes, signed-reference reproduction, exact locally accepted forms, and the parent F9 exit gate remain open.
...
| Reporting/templates | Governed reporting, macro-free workbook preflight, and F9.1 Budget accountability/trial-balance controls, source drill-through, applicability gates, and reproduction receipts exist | Extend the finance form catalog, signed-reference reproduction, financial statements/notes, dashboards, and exact template acceptance through F9–F10. |
