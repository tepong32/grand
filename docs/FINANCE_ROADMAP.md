...
| Standalone Accounting | Fiscal/fund/account setup, controlled opening intake/posting/reconciliation/export, F7.1 rule-backed JEVs, F7.2 subsidiary controls, F7.3 payment-event JEVs, F7.4 governed close/reopen, and F8.1 remittance JEV orchestration implemented | Continue under F7–F9; validate the local close policy, broaden required subsidiary ledgers, and complete accepted statements/tax outputs. |
...
| Reporting/templates | Governed reporting, macro-free workbook preflight, F9.1 evidence/reproduction controls, the F9.2 operational catalog, F9.3 governed statement composition/explained measures, F9.4 note/reference controls, and F10.1 governed template promotion/rollback exist | Confirm accepted tax outputs, reproduce a complete signed package, and complete exact local-form, printer, golden-output, and named-office acceptance through F9–F10. |
