...
|---|---|---|
| Governance and discovery | F0.1 field templates, F0.2 governed Decisions & evidence register, F0.3 editable cycle coverage/acceptance-example controls, and F0.4 filtered department export implemented | Populate and extend the starters from actual interviews/current sources, resolve every enabled-scope gap, and obtain independent LGU confirmation; generated prompts and portable indexes cannot satisfy cutover readiness. |
...
1. **F0.1–F0.4 — Finance evidence register, interview kit, governed decisions, editable coverage, and portable register — implemented preparation control**: repository-safe field templates plus an in-app maker–checker register, nine non-authoritative candidate-cycle prompts, retained acceptance examples, exact-scope blockers, reasoned successors, visible coverage, cycle-linked cutover gating, floating guidance, and audited per-record/filtered-department TraceSync exports; see [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md). Actual field evidence and parent-F0 acceptance remain open.
...
The [roadmap completion audit](FINANCE_ROADMAP_COMPLETION_AUDIT.md) records the current F0–F11 evidence, the clean 397-test two-store regression result, the missing LGU field proofs, and the dependency-ordered path from discovery closure through final cutover authority.
