...
Implementation position: F9.1 supplies the immutable evidence and reproducibility foundation, while F9.2 adds the Budget/Accounting/Treasury operational catalog. F9.3 adds governed financial-position/performance composition, exact account coverage, visible ledger equations, source drill-through, and explained measures; see [Finance accountability reporting](FINANCE_ACCOUNTABILITY_REPORTING.md), [Finance operational report catalog](FINANCE_OPERATIONAL_REPORT_CATALOG.md), and [Finance statement controls](FINANCE_STATEMENT_CONTROLS.md). Statement notes, current accepted tax/BIR outputs, signed-reference reproduction, exact locally accepted forms, named-office acceptance, and the parent F9 exit gate remain open.
...
| Reporting/templates | Governed reporting, macro-free workbook preflight, F9.1 evidence/reproduction controls, the F9.2 operational catalog, and F9.3 governed statement composition/explained measures exist | Extend to statement notes, accepted tax outputs, signed-reference reproduction, remaining catalog controls, and exact template acceptance through F9–F10. |
