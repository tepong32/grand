...
Implementation position: F10.1 adds inactive human-editable versions, safe preview generation, automatic same-period golden data/control comparison, plain-language form/print checks, template diffs, schedule-impact review, independent approval, deployment-free activation, reasoned rollback, append-only events, department roles, inline guidance, and TraceSync-ready receipts; see [Finance visual template promotion and rollback](FINANCE_TEMPLATE_PROMOTION.md). Exact local forms, optional-block behavior, physical printer/form-stock trials, accepted golden outputs, and named-office sign-off remain open, so the parent F10 exit gate is not yet claimed.
...
| Reporting/templates | Governed reporting, macro-free workbook preflight, F9.1 evidence/reproduction controls, the F9.2 operational catalog, F9.3 governed statement composition/explained measures, F9.4 note/reference controls, F9.5 governed tax detail/summary schedules, and F10.1 governed template promotion/rollback exist | Confirm accepted tax forms, filing/remittance evidence, and TIN custody; reproduce a complete signed package; and complete exact local-form, printer, golden-output, and named-office acceptance through F9–F10. |
| Shadow operation and cutover | F11.1 governed cycles/cutover, F11.2 source staging/drift controls, F11.3 approved cadence/run/defect gates, F11.4 curriculum/support plus witnessed readiness exercises, and F11.5 consecutive-cycle/signed-reference gates implemented | Confirm local plan/content/pass-condition values, any production adapter, complete actual exercises and field cycles, accepted local forms, and actual seven-party/cutover records before claiming authority. |
