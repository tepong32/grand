...
| Voucher Workbench | Shadow vertical slice through release plus F5.1 obligation handoff and F5.2 typed documentary-readiness gate implemented | Continue F5 payable relationships/recognition, exports, per-variant replay, and acceptance without restoring a second budget balance. |
...
| Annual budget, appropriation, allotment, and obligation registry | F3.1–F4.2 implemented; F5.1–F5.2 consume and recheck the authoritative obligation UUID/checksum | Reconcile accepted schedules/forms and complete F5 relationships, exports, and replay before official use. |

The earlier statement that printing was the first confirmed voucher mismatch remains true only inside the implemented voucher subcycle. From a complete-cycle perspective, F5.1 covers the one-to-one ordinary-supplier obligation handoff and F5.2 supplies governed variant/checklist infrastructure plus independent readiness review. GRAND still diverges inside F5 at locally accepted per-variant rules, payable relationship/recognition decisions, controlled exports, exceptions, forms, and redacted replay.
...
10. **F5.2 — Transaction variants and documentary readiness — implemented synthetic control**: Finance Setup governs typed variants and conditional/waiver rules; the requesting office resolves pinned evidence and a separate Accounting reviewer accepts or returns the same case. Parent F5 relationships, exports, local acceptance, and replay remain open; see [Finance transaction variants and payable readiness](FINANCE_PAYABLE_VARIANTS.md).
11. **F5.3 — Payable relationships, recognition, reconciliation, and exports**: model accepted one-to-many/many-to-one/partial/progress/final relationships, recognition/adjustment decisions, control-total reconciliation, and TraceSync-ready transaction exports.
12. **F6.1 — Controlled print and finance custody**: print/version/reprint states, packet manifest, TracePoint creation/linkage, wet-signature checkpoints, returned-packet gate, and redacted output comparison.

Delivery status: the repository-safe templates for **F0.1** are available in the [Finance evidence register and interview kit](finance-discovery/README.md). The **F1.1** design contract and clickable synthetic UX are available in the [complete-cycle Finance information architecture](finance-ia/README.md). The **F2.1** software foundation is documented in [Finance fiscal-year and classification foundation](FINANCE_FISCAL_FOUNDATION.md), the **F2.2** controls are documented in [Finance opening balances and control-total intake](FINANCE_OPENING_BALANCES.md), the implemented **F3.1–F3.2** preparation and operational-authorization controls are documented in [Finance annual budget preparation](FINANCE_ANNUAL_BUDGET.md), **F4.1** in [Finance allotment release control](FINANCE_ALLOTMENT_CONTROL.md), **F4.2** in [Finance obligation control](FINANCE_OBLIGATION_CONTROL.md), **F5.1** in [Finance payable intake](FINANCE_PAYABLE_INTAKE.md), and **F5.2** in [Finance transaction variants and payable readiness](FINANCE_PAYABLE_VARIANTS.md). These deliver reviewable instruments and implemented synthetic controls, not the parent phase acceptance gates; field evidence, accepted local schedules/forms, and named-office reconciliation/acceptance remain required.
