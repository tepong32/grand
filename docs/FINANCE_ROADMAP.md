...
Implementation position: native MySQL logical backup creation now produces one verified, atomically published, checksum-manifested recovery set for both the default and Finance stores, with guarded retention, a separately retainable manifest hash, strict read-only copied-set verification, and an operator restore runbook. A non-root Python 3.11/Gunicorn container, WhiteNoise static delivery, two-store environment contract, stdout logging, and minimal health probe provide the deployable runtime foundation. Actual Render topology, persistent/shared storage, backup publication schedule, off-host copying, encryption/retention decisions, monitoring, and a witnessed two-store restore rehearsal with reconciled control totals remain F1/cutover acceptance gates; a built image, integrity receipt, or created artifact is not represented as production- or restore-tested.
