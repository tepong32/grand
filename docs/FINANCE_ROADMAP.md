...
Implementation position: F8.1–F8.4 synthetic controls are implemented. F8.1 covers cross-voucher withholding remittance through posted liability reduction. F8.2 covers checksummed monthly bank-statement versions, exact/guided posted-GL matching, adjusted-balance timing items, independent zero-difference reconciliation, Internal How-Tos, and portable evidence. F8.3 adds locally versioned Observe/Enforce cash policies, independently approved reconciliation-backed positions, issue reservations, configurable unclaimed/stale/returned classification, department guidance, and portable evidence. F8.4 adds retained multi-case advice versions, independent review, actual bank submission and acknowledgement/return evidence, acknowledgement-gated release, reasoned successors, and returned-payment reversal/reissue orchestration; see [Finance remittance execution](FINANCE_REMITTANCE_EXECUTION.md), [Finance bank reconciliation](FINANCE_BANK_RECONCILIATION.md), [Finance cash position and instrument ageing](FINANCE_CASH_POSITION.md), and [Finance bank advice and returned instruments](FINANCE_BANK_ADVICE.md). Prior-item carry-forward, exact official forms/rules, and full redacted instrument-to-bank replay remain acceptance/delivery gates.
...
| Treasury | Check registration, cancellation/replacement, F7.3 payment posting, F8.1 remittance, F8.2 bank reconciliation, F8.3 cash position/reservation/ageing, and F8.4 advice acknowledgement plus returned-payment orchestration implemented | Validate official advice/cash/check forms, entries, channels, and thresholds; add prior-item carry-forward and named-office replay in F8. |
