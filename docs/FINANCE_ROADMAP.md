...
Implementation position: F1.2 adds one plain Finance entry page whose cards are composed only from existing Budget, Voucher, Accounting, Reporting, Finance setup/discovery, and field-operation access checks. F1.3 adds a controlled search handoff into the existing permission-filtered Voucher Workbench and shared voucher-to-release timeline; it does not create a second case index, disclose hidden-case counts or suggestions, or satisfy the broader pre-voucher My Work/search/notification contract. Reporting alone does not classify a general employee as Finance; a named cross-office field reviewer receives only the applicable field destination; existing direct links remain available; and each destination rechecks its own scope. A department-seeded floating guide explains shared-case handoffs, governed corrections, exports, and private tutorial progress. Native MySQL logical backup creation produces one verified, atomically published, checksum-manifested recovery set for both the default and Finance stores, with guarded retention, a separately retainable manifest hash, strict read-only copied-set verification, and an operator restore runbook. A non-root Python 3.11/Gunicorn container, WhiteNoise static delivery, two-store environment contract, stdout logging, minimal health probe, and non-secret configuration/live production preflight provide the deployable runtime foundation. Actual named-user role walkthroughs, complete-cycle My Work/search/notification acceptance, Render topology, persistent/shared storage, backup publication schedule, off-host copying, encryption/retention decisions, monitoring, and a witnessed two-store restore rehearsal with reconciled control totals remain F1/cutover acceptance gates; a visible card, search result, built image, preflight/integrity receipt, or created artifact is not represented as authority, production acceptance, or restore proof. See [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md).
...
|---|---|---|
| Finance operations entry | F1.2 permission-shaped landing page and department-seeded contextual guide plus F1.3 permission-filtered shared voucher-to-release case finder implemented over the existing domain workspaces | Perform named-user desktop/narrow-layout and accessibility review; complete the broader pre-voucher/cross-cycle My Work, saved-view, search, and notification contract without duplicating domain queues or weakening their access controls. |
...
2. **F1.1–F1.3 — Complete-cycle information architecture, permission-shaped entry, and narrow shared-case finder — implemented design and bounded controls**: the role/permission matrix, case/timeline/search/status contract, and synthetic prototype remain the design baseline; `/finance/` now provides one plain starting page composed from existing workspace guards, with direct-link compatibility, contextual guidance, and a controlled-reference search handoff into the existing permission-filtered Voucher Workbench. Named-user walkthroughs, broader pre-voucher/cross-cycle My Work, saved-view, search, and notification behavior, supported-device/accessibility observation, and parent-F1 acceptance remain open; see [complete-cycle Finance information architecture](finance-ia/README.md) and [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md).
...
F1.2–F1.3 delivery is recorded separately in [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md); it implements the permission-shaped navigation slice and the existing shared voucher-to-release case-finder slice of the F1.1 information-architecture contract while leaving broader pre-voucher/cross-cycle My Work, saved-view, search, notification, and field-acceptance work open.
