...
Implementation position: F11.1 adds limited shadow/parallel cycle plans, checksum- and schema-signature-locked source references, exact case/batch/period/register/ledger/report comparison rows, blocking owned defects, immutable submission evidence, independent reconciliation, named seven-party training/UAT acceptance, exact-scope cutover records, separate authority decisions, retained rollback, contextual Internal How-Tos, and TraceSync-ready evidence packages; see [Finance shadow operation, UAT acceptance, and controlled cutover](FINANCE_SHADOW_CUTOVER.md). Automatic read-only source adapters, scheduled daily reconciliation, locally accepted escalation/curricula/runbooks, named non-functional exercises, consecutive field cycles, and actual stakeholder/cutover signatures remain open, so the parent F11 exit gate is not claimed.
...
| Shadow operation and cutover | F11.1 governed cycles, comparisons, stakeholder decisions, exact-scope authority, rollback, guidance, and portable evidence implemented | Confirm source adapters/cadence, curricula/runbooks, non-functional exercises, consecutive local field cycles, and actual seven-party approval before claiming authority. |
