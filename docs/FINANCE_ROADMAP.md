...
Implementation position: F8.1–F8.3 synthetic controls are implemented. F8.1 covers cross-voucher withholding remittance through posted liability reduction. F8.2 covers checksummed monthly bank-statement versions, exact/guided posted-GL matching, adjusted-balance timing items, independent zero-difference reconciliation, Internal How-Tos, and portable evidence. F8.3 adds locally versioned Observe/Enforce cash policies, independently approved reconciliation-backed positions, issue reservations, configurable unclaimed/stale/returned classification, department guidance, and portable evidence; see [Finance remittance execution](FINANCE_REMITTANCE_EXECUTION.md), [Finance bank reconciliation](FINANCE_BANK_RECONCILIATION.md), and [Finance cash position and instrument ageing](FINANCE_CASH_POSITION.md). Advice submission/acknowledgement/return, returned-item Accounting orchestration, prior-item carry-forward, exact official forms, and full redacted instrument-to-bank replay remain acceptance/delivery gates.
...
| Treasury | Check registration, cancellation/replacement, advice/release controls, F7.3 payment posting, F8.1 remittance execution, F8.2 bank reconciliation, and F8.3 reconciliation-backed cash position/reservation/ageing controls implemented | Validate official cash/check reports and thresholds; add advice acknowledgement, returned-item Accounting orchestration, prior-item carry-forward, and named-office replay in F8. |
