...
Implementation position: F1.2 adds one plain Finance entry page whose cards are composed only from existing Budget, Voucher, Accounting, Reporting, Finance setup/discovery, and field-operation access checks. Reporting alone does not classify a general employee as Finance; a named cross-office field reviewer receives only the applicable field destination; existing direct links remain available; and each destination rechecks its own scope. A department-seeded floating guide explains shared-case handoffs, governed corrections, exports, and private tutorial progress. Native MySQL logical backup creation produces one verified, atomically published, checksum-manifested recovery set for both the default and Finance stores, with guarded retention, a separately retainable manifest hash, strict read-only copied-set verification, and an operator restore runbook. A non-root Python 3.11/Gunicorn container, WhiteNoise static delivery, two-store environment contract, stdout logging, minimal health probe, and non-secret configuration/live production preflight provide the deployable runtime foundation. Actual named-user role walkthroughs, complete-cycle search/notification acceptance, Render topology, persistent/shared storage, backup publication schedule, off-host copying, encryption/retention decisions, monitoring, and a witnessed two-store restore rehearsal with reconciled control totals remain F1/cutover acceptance gates; a visible card, built image, preflight/integrity receipt, or created artifact is not represented as authority, production acceptance, or restore proof. See [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md).
...
|---|---|---|
| Finance operations entry | F1.2 permission-shaped landing page and department-seeded contextual guide implemented over the existing domain workspaces | Perform named-user desktop/narrow-layout and accessibility review; complete the broader cross-cycle My Work/search/notification contract without duplicating domain queues or weakening their access controls. |
...
2. **F1.1–F1.2 — Complete-cycle information architecture and permission-shaped entry — implemented design and narrow navigation control**: the role/permission matrix, case/timeline/search/status contract, and synthetic prototype remain the design baseline; `/finance/` now provides one plain starting page composed from existing workspace guards, with direct-link compatibility and contextual guidance. Named-user walkthroughs, wider My Work/search/notification behavior, supported-device/accessibility observation, and parent-F1 acceptance remain open; see [complete-cycle Finance information architecture](finance-ia/README.md) and [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md).
...
F1.2 delivery is recorded separately in [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md); it implements the permission-shaped navigation slice of the F1.1 information-architecture contract while leaving wider My Work/search/notification and field-acceptance work open.
