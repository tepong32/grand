...
| Voucher Workbench | Shadow vertical slice through release plus F5.1 authoritative obligation/payable entry implemented | Continue F5 transaction variants, documentary rules, relationship patterns, and acceptance without restoring a second budget balance. |
...
| Annual budget, appropriation, allotment, and obligation registry | F3.1–F4.2 implemented; F5.1 consumes authoritative obligation UUID/checksum | Reconcile accepted schedules/forms and complete F5 variants and evidence before official use. |

The earlier statement that printing was the first confirmed voucher mismatch remains true only inside the implemented voucher subcycle. From a complete-cycle perspective, F5.1 now covers the one-to-one ordinary-supplier obligation handoff, but GRAND still diverges inside F5 at accepted documentary rules, payable relationships/decisions, variants, exceptions, forms, and redacted replay.
