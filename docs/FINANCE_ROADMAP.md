...
Implementation position: native MySQL logical backup creation now produces one verified, atomically published, checksum-manifested recovery set for both the default and Finance stores, with guarded retention and an operator restore runbook. A non-root Python 3.11/Gunicorn container, WhiteNoise static delivery, two-store environment contract, stdout logging, and minimal health probe provide the deployable runtime foundation. Actual Render topology, persistent/shared storage, backup publication schedule, off-host copying, encryption/retention decisions, monitoring, and a witnessed two-store restore rehearsal with reconciled control totals remain F1/cutover acceptance gates; a built image or created artifact is not represented as production- or restore-tested.
...
Implementation position: F10.1 adds inactive human-editable versions, safe preview generation, automatic same-period golden data/control comparison, plain-language form/print checks, template diffs, schedule-impact review, independent approval, deployment-free activation, reasoned rollback, append-only events, department roles, inline guidance, and TraceSync-ready receipts. F10.2 adds a department-bounded plain-language form register, dynamic required/optional/conditional/repeating sections, exact promoted-report or active-preflighted-workbook source locks, seven independently witnessed practical tests, maker–checker local acceptance, reasoned fully retested successors, historical reproducibility, guidance, and TraceSync-ready evidence; see [Finance visual template promotion and rollback](FINANCE_TEMPLATE_PROMOTION.md) and [Finance local-form inventory and acceptance](FINANCE_LOCAL_FORM_ACCEPTANCE.md). Actual LGU forms, accepted authorities/decisions, golden outputs, devices, physical printer/form-stock trials, retained samples, and named-office sign-off remain open, so the parent F10 exit gate is not yet claimed.
...
| Reporting/templates | Governed reporting, macro-free workbook preflight, F9.1 evidence/reproduction controls, the F9.2 operational catalog, F9.3 governed statement composition/explained measures, F9.4 note/reference controls, F9.5–F9.6 governed tax schedules/filing evidence, F9.7 accountability-package assembly, F10.1 template promotion/rollback, and F10.2 local-form acceptance controls exist | Populate actual form inventories and accepted package profiles; perform the witnessed golden/output/accessibility/printer/recovery trials; confirm filing/remittance evidence and TIN custody; reproduce a complete signed/acknowledged package; and obtain named-office acceptance through F10–F11. |
| Shadow operation and cutover | F11.1 governed cycles/cutover, F11.2 source staging/drift controls, F11.3 approved cadence/run/defect gates, F11.4 curriculum/support plus witnessed readiness exercises, F11.5 consecutive-cycle/signed-reference gates, and F11.6 exact accepted-form lineage implemented | Populate and accept actual F10.2 forms, select every applicable form in the qualification plan, confirm local plan/content/pass-condition values, any production adapter, complete actual exercises and field cycles, and obtain actual seven-party/cutover records before claiming authority. |
...
30. **F10.2 — Governed local-form inventory and acceptance — implemented synthetic control**: plain-language form catalogs, dynamic required/optional/conditional/repeating sections, exact activated source locks, seven maker–witness practical tests, independent form acceptance, reasoned fully retested successors, immutable history, department guidance, and TraceSync-ready evidence; see [Finance local-form inventory and acceptance](FINANCE_LOCAL_FORM_ACCEPTANCE.md). Actual LGU references, authority and acceptance decisions, accepted golden outputs, physical devices/stock, retained field samples, and named-office sign-off remain parent-F10 gates.
