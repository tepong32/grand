...
Implementation position: F0.1 provides repository-safe interview, evidence, actual-step, role/signature, decision, and redacted-replay worksheets. F0.2 adds a plain-language in-app Decisions & evidence register with the five evidence labels, named cross-office owner/reviewer access, immutable submitted snapshots and checksums, recorded unresolved scope blockers, reasoned successors, an Accounting-specific floating guide, and audited TraceSync CSV export. A linked cycle cannot pass cutover readiness without one current independently recorded LGU-confirmed F0 decision for its exact enabled scope and no current linked blocker; see [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md). Actual interviews, local sources, custody records, applicability decisions, and named-office acceptance remain open, so the parent F0 exit gate is not claimed.
...
|---|---|---|
| Governance and discovery | F0.1 field templates plus F0.2 governed Decisions & evidence register, exact-scope blockers, successor history, cycle linkage, floating guidance, and TraceSync export implemented | Populate the register from actual interviews/current sources, resolve every enabled-scope gap, and obtain independent LGU confirmation; an empty register cannot satisfy cutover readiness. |
...
1. **F0.1/F0.2 — Finance evidence register, interview kit, and governed decisions — implemented preparation control**: repository-safe evidence/decision templates, authority labels, transaction catalog, role/signature matrix, actual-step worksheet, redacted replay checklist, plus an in-app maker–checker register with exact-scope blockers, reasoned successors, cycle-linked cutover gating, floating guidance, and TraceSync export; see [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md). Actual field evidence and parent-F0 acceptance remain open.
...
The [roadmap completion audit](FINANCE_ROADMAP_COMPLETION_AUDIT.md) records the current F0–F11 evidence, the clean 393-test two-store regression result, the missing LGU field proofs, and the dependency-ordered path from discovery closure through final cutover authority.
