...
|---|---|---|
| Finance operations entry | F1.2 permission-shaped landing page, F1.3 shared voucher-to-release case finder, F1.4 private current-scope-rechecked saved case views, and F1.5 exact-drill permission-filtered attention overview foundation implemented over existing domain workspaces | Add the deferred source adapters and stable item-level task contract; perform named-user desktop/narrow-layout and accessibility review; complete governed shared views, broader search, following, and notifications without duplicating domain queues or weakening access controls. |
...
F1.2–F1.5 delivery is recorded in [Finance operations entry](FINANCE_OPERATIONS_ENTRY.md) and [Finance work-attention foundation](FINANCE_MY_WORK.md); it implements permission-shaped navigation, the existing shared voucher-to-release case finder, private current-scope-rechecked saved views, and exact-drill supported attention groups while leaving deferred adapters, the stable item-level cross-cycle task contract, governed shared views, broader search, following/notifications, and field acceptance open.
