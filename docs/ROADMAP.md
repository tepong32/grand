...
The [GRAND Finance complete-cycle roadmap](FINANCE_ROADMAP.md) is the canonical plan for the long-term Finance sub-application. It replaces the earlier assumption that the next material step is only an ordinary-supplier voucher pilot. The implemented F10.1 control is documented in [Finance visual template promotion and rollback](FINANCE_TEMPLATE_PROMOTION.md), and the F11.1–F11.4 transition, source-staging, scheduled-reconciliation, defect, curriculum/support, and witnessed-readiness controls are documented in [Finance shadow operation, UAT acceptance, and controlled cutover](FINANCE_SHADOW_CUTOVER.md).
