...
F5.1 now supplies the authoritative obligation-to-payable/voucher handoff for a synthetic one-to-one ordinary-supplier case. The first incomplete link remains the rest of F5: accepted documentary rules, relationship patterns, transaction variants, forms, exceptions, and redacted end-to-end replay. Local signed-schedule/form reconciliation and acceptance remain mandatory before official use; printing remains the earliest confirmed mismatch only within the implemented voucher subcycle.
