...
F5.1 supplies the authoritative obligation-to-payable/voucher handoff for a synthetic one-to-one ordinary-supplier case. F5.2 adds configurable transaction variants, pinned documentary rules, requesting-office decisions, and independent Accounting accept/return. The first incomplete link remains payable relationship/recognition patterns, controlled exports, accepted per-variant rules/forms, exceptions, and redacted end-to-end replay. Local signed-schedule/form reconciliation and acceptance remain mandatory before official use; printing remains the earliest confirmed mismatch only within the implemented voucher subcycle.
