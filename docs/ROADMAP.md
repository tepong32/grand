...
F0.2–F0.4 add the governed [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md): five explicit evidence labels, named cross-office owner/reviewer access, immutable submitted snapshots, exact-scope unresolved blockers, reasoned successors, nine editable candidate-cycle coverage prompts, retained acceptance examples, visible missing-area guidance, per-record and filtered department TraceSync exports, and detailed plus whole-scope discovery gates before cutover. They structure and safeguard local decisions but do not manufacture or imply missing LGU evidence.
