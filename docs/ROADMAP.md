...
F0.2 adds the governed [Finance decisions and evidence register](FINANCE_DISCOVERY_DECISIONS.md): five explicit evidence labels, named cross-office owner/reviewer access, immutable submitted snapshots, exact-scope unresolved blockers, reasoned successors, contextual guidance, TraceSync export, and an exact-scope discovery gate before cutover. It structures local decisions but does not manufacture or imply missing LGU evidence.
