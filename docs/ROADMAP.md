...
The first unimplemented complete-cycle link is now governed F5 request/procurement/delivery/payable intake and authoritative obligation-to-voucher integration. F3.1–F4.2 provide synthetic annual-budget, appropriation, allotment, obligation, and RAAO-equivalent controls, but local signed-schedule/form reconciliation and acceptance remain mandatory before official use. Printing remains the earliest confirmed mismatch only within the implemented voucher subcycle.
