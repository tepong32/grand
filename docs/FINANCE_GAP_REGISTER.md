...
- Severity/status: **CRITICAL — OPEN**, identified during v0.7.3 source audit. Dependent advice coverage progression is stopped pending reproduction and remediation; production remains NO-GO.
- Code evidence: `review_advice`, `record_advice_submission` and `record_bank_response` load a batch by primary key through `_lock_batch`, which currently checks version but not actor office. `_require` checks explicit permission but does not exclude UAT actors. Returned-instrument decision custody requires the same investigation.
