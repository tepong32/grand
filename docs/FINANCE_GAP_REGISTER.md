...
- Process/module: native two-store backup and Finance recovery.
- Severity/status: **CRITICAL - VERIFIED** in v0.7.69 for the supported fixed single-server topology. Overall scrutiny and production remain NO-GO.
- Reproduction: on disposable MySQL 8.4.12, the actual native writer finished the default dump, a real synthetic voucher service chain created and posted a new recognition, then the writer captured Finance. Set `20260909T122843374118Z-00885081` was published as complete and passed retained-hash verification. Native restore contained posted entry `64df478a-b460-4f07-b720-9c65de32888e` (debit/credit 1,000.00), but lacked its request `46f4aba0-3b97-42d4-88e9-2c0e3381f69b` and case `3325f375-cc25-4751-bbf6-d5da51c38dca`.
- Cause: implementation/backup capture architecture. Separate `--single-transaction` invocations provide separate snapshots; the filesystem backup-job lock does not stop application transactions. File hashes establish artifact integrity, not a common relational recovery point.
- Evidence: ignored `.tmp/native_backup_interleaving.py`, `.tmp/native_read_interleaving.py`, `.tmp/finance-scrutiny-native-interleaving.log` and the synthetic `interleaving-receipt.json` / `interleaving-finding.json`. No production data or real credentials used. The preceding idle-source restore passed but does not mitigate this demonstrated concurrent-write failure.
- Required next action: implement a fail-closed coordinated native capture boundary, preserve separate artifacts/stores, reproduce blocked or coherent concurrent posting, restore and reconcile again, then run applicable backup, authorization/integrity and full-chain regressions. Do not relabel the prior artifact as a verified recovery point or accept this risk autonomously.
- Remediation obtained: one dedicated MySQL global-read-lock session spans both dumps, with same-server UUID validation, connection-identity checks, bounded acquisition and fail-closed publication. Separate schemas/artifacts remain. Missing privilege and killed-session native attempts publish nothing; previous sets survive and clean retry passes. A real service writer was observed waiting for the global read lock and completed after release. The coordinated restore matched all 203 pre-capture table controls, retained both voucher/request/JEV chains, reconciled debit/credit 2,000.00 and contained zero orphaned postings. Seven added backup tests cover failure/publication/legacy metadata boundaries. PASS: final full SQLite suite, 708 tests in 314.043 seconds; final full MySQL 8.4.12 suite, 708 tests in 854.600 seconds. Both aliases remained separate. System, migration-drift, compilation and diff checks passed. The final affected native suite also passed 178 tests in 182.916 seconds. See the [scrutiny report](FINANCE_SCRUTINY_2026-09-09.md) and D-060 for operational constraints.

## FIN-GAP-033 - Authoritative payable projection exceeds native field capacity

- Severity/status: **CRITICAL - VERIFIED** in v0.7.69. Width 64 and a text source-reference field retain the complete unchanged identities. PASS: final full SQLite suite, 708 tests in 314.043 seconds; final full MySQL 8.4.12 suite, 708 tests in 854.600 seconds. Both aliases remained separate. System, migration-drift, compilation and diff checks passed. The final affected native suite also passed 178 tests in 182.916 seconds.
- Reproduction: the 706-test native run (855.461 seconds) produced `DataError 1406` for `BudgetObligation.source_kind`; the model permits 24 characters while existing single/relationship projection identifiers require 27/40. Three native failures affect initial binding, consolidated allocation and the authoritative F2-F9 replay. SQLite accepted the longer values and its 706-test run passed.
- Evidence of pre-existing cause: checkpoint `d809977` already has the same 24-character field and projection strings. This is not caused by the new backup code. Logs: `.tmp/finance-scrutiny-native-v069-first-full.log`.
- Required remediation: widen the persisted field without rewriting historical identifiers, add the migration and rerun all three original native failures plus dependent/full regressions. No live migration is authorized by this synthetic investigation.
- Extended capacity reproduction: a valid four-obligation consolidated claim also fails with native `DataError 1406` on the 160-character `budget_source_reference`. The original UUID list must be retained in full, not truncated. One native four-source test reproduced this in 3.209 seconds (`.tmp/finance-scrutiny-native-four-obligations.log`); extend the same capacity repair and add a retained four-source regression.

## FIN-GAP-034 - MySQL omits obligation identity uniqueness

- Severity/status: **CRITICAL - VERIFIED** in v0.7.69. Generated nullable keys now enforce the intended request, number and active-allocation identities. PASS: final full SQLite suite, 708 tests in 314.043 seconds; final full MySQL 8.4.12 suite, 708 tests in 854.600 seconds. Both aliases remained separate. System, migration-drift, compilation and diff checks passed. The final affected native suite also passed 178 tests in 182.916 seconds.
- Reproduction: `AnnualBudgetPreparationTests.test_obligation_rejects_control_difference_excess_and_duplicate_request` created a duplicate original request instead of raising `IntegrityError` on MySQL. Django warned that the conditional unique constraints are unsupported; the conditions are present at `d809977` and therefore pre-existing. The numbered-obligation constraint has the same unsupported form and is included in remediation scope.
- Required remediation: database-generated nullable identity keys with ordinary unique constraints, preserving blank-number drafts and correction references. Fail migration before schema changes if historical duplicates require controlled investigation; never silently deduplicate financial records. Verify both inserts and updates on SQLite/MySQL and rerun the Budget/payable/authoritative chain.
- Extended reproduction: after the width and request/number repairs, the original consolidated-payable test progressed to its active-allocation duplicate check, which also failed on MySQL. The same portable-key remediation now covers the active obligation/case allocation identity while retaining superseded/cancelled versions. The first rerun had 3 passes and 1 failure in 28.832 seconds; the subsequent five-test native reproduction/identity run passed in 30.021 seconds. Final regression passed; generated-key restore and authoritative report reproduction also passed. See the scrutiny report.

## FIN-GAP-035 - Concurrent bank matching exposes an unhandled deadlock

- Severity/status: **MEDIUM - VERIFIED** in v0.7.70. The observed transaction rolls back and exactly one active match survives; duplicate financial records were not reproduced by this experiment.
- Reproduction: two independently staged synthetic statement batches concurrently match the same genuinely posted 100.00 bank line through `match_bank_statement_row`. Both reach insertion before the outstanding-item query upgrades their shared journal-line locks. Native MySQL rolls back one with error 1213. The first probe checked only the surviving match count and passed (1 test, 6.129 seconds); it is not a passing user-workflow result.
- User-visible verification: two authenticated requests to the actual bank-match endpoint produced HTTP 500 and HTTP 302, retaining one active match. The endpoint catches ValidationError but not the native OperationalError. The HTTP regression failed (1 test, 6.792 seconds); ignored logs `.tmp/finance-scrutiny-native-bank-match-probe.log` and `.tmp/finance-scrutiny-native-bank-http-probe.log`, with native lock evidence in `.tmp/finance-scrutiny-20260909/bank-match-deadlock.txt`.
- Cause: concurrent transaction handling and lock acquisition. The native conditional unique constraints are also absent, so this one deadlock outcome must not be described as proof of database uniqueness across all paths.
- Safe interim action: reload the statement and inspect the retained match before retrying. The service rejects an already committed competing match. No production acceptance follows from this workaround.
- Closure: Bank matching now reports known conflicts after complete rollback, with an explicit reload/check/retry instruction and no implicit replay. Generated nullable keys enforce active bank-row and journal-line match identities while preserving superseded history. Migration accounting/0011 refuses historical duplicates before DDL. A committed fresh two-store MySQL runner and independent CI service job complement SQLite. PASS: full SQLite run, 711 tests discovered in 355.841 seconds (710 executed; one MySQL-only case skipped); full MySQL 8.4.12 run through the committed native runner, all 711 tests in 767.251 seconds. System, migration-drift, compilation and diff checks passed. Native duplicate-data preflight and final-schema restore/report reproduction passed. See [the validation report](FINANCE_BANK_MATCH_VALIDATION_2026-09-09.md) for exact evidence and limits.
...
## FIN-GAP-029 - Voucher controls diverged from source actions

- Process/module: preparation, payable review, custody/payment controls, pending amendments and generic signature-stage attention.
- Severity/status: **MEDIUM - VERIFIED**, known findings repaired across v0.7.58-60.
- Original evidence: generic grants exposed UAT, wrong-office and maker controls; signature choices ignored order, pending amendments invited duplicates, print/packet steps omitted source dependencies, and payment defaults/choices diverged from retained recovery states.
- Expected behavior: preserve separately authorized reads and source readiness explanations while displaying source-authorized actions. Preserve pinned-owner artifact/amendment authority across custody and existing governed exemptions.
- Closure: v0.7.58 repaired preparation/review/signature/UAT/pending-amendment controls. v0.7.59 aligned print preparation/recording/packet selectors, retained reasoned replacements and repaired missing-owner page readability. v0.7.60 repaired current-office payment controls, terminal links/cancellation, returned-check defaults, release exceptions, duplicate advice preparation and generic signature-stage attention.
- Verification: Five reproductions produced ten failed assertions across two runs (8.231 and 5.681 seconds). PASS: 177 voucher/custody/My Work/operations tests in 96.163 seconds. PASS: 692 full-project tests in 248.224 seconds. System, migration-drift, compilation and diff checks passed. The first focused run failed one new test because its added Accounting issue grant changed the fixture routing office; the test now removes that grant before source issuance and asserts the retained Treasury destination. This was FAIL - CAUSED BY CURRENT WORK (test fixture), repaired before final verification. Full regression covers the shared workbench filter, queue/export dependencies and payment recovery workflows. Browser NOT RUN - OUT OF SCOPE for conditional controls and field choices without layout changes; rendered-page and real recovery tests verify the change. No mutation-service/schema changes or live migration.
- Scope: this verifies the recorded findings, not a claim of bug-free Finance or production acceptance. Detailed print/packet/amendment and posting/reopen personal history remains in the coverage roadmap.
...
- Severity/status: **HIGH - VERIFIED**, reproduced and repaired in v0.7.57.
...
- Expected behavior: preserve those existing across-custody owning-Accounting workflows while excluding unrelated offices with generic grants. Resolve authority from the stored case before writes/replay and item visibility from the stored item/current packet before new association; preserve immutable files, source amounts and existing packet-visibility policy. Replay returns the retained association and cannot substitute a caller-supplied item.
- Closure: a shared pinned-owner predicate now governs source services and page controls; stored case checks precede replay, and stored item/current-packet checks precede new linking.

- Verification: three reproduction tests produced seven failures in 4.682 seconds. All 90 focused voucher/custody tests passed in 60.994 seconds. All 684 full-project tests passed in 407.127 seconds. System, migration-drift, compilation and diff checks passed. Forged owner/packet inputs, stale association, HTTP denial, UAT and existing across-custody owner workflows are covered.

## FIN-GAP-031 — Field source-layout review discoverability

- Process: F11 source staging and independent schema-drift review. Severity/status: **HIGH — VERIFIED** in v0.7.65.
- Reproduced: an eligible independent reviewer has no exact My Work task for a current pending drift source; the real source-staging regression failed (1 test, 2.249 seconds). The cycle page additionally displayed a review link to the source stager although the service rejects self-review.
- Remediation: shared read-only eligibility for current pending drift in a draft cycle, acting office, explicit independent reviewer and UAT exclusion. Use it in register attention, counts, exact tasks and source controls. Preserve existing service revalidation and immutable source versions.
- Verification: PASS: 133 cutover/My Work/operations tests in 101.312 seconds; system, migration-drift, compilation and diff checks passed. Initial reproduction failed one real source-staging task assertion (1 test, 2.249 seconds). Tests cover independent task/source navigation, self-review exclusion, acceptance/start, rejected-source replacement, supersession, UAT and permission loss. Full suite NOT RUN - OUT OF SCOPE for this read-only selector and form-eligibility refinement: affected field lifecycle and shared work/operations suites cover the changed contracts, with v0.7.64 full-project baseline recorded separately. Browser NOT RUN - OUT OF SCOPE: existing layout unchanged; rendered links and form responses verified by integration tests. No schema/service mutation changes or live migration; user database unchanged. The first 133-test affected run errored on a missing import in the new external-lock test (100.629 seconds; FAIL - CAUSED BY CURRENT WORK, test setup); the import was corrected and the entire affected suite rerun. Production remains NO-GO; see the operational scrutiny gate and D-055.
