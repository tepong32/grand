...
- Severity/status: **MEDIUM - PARTIALLY REPAIRED**, v0.7.58 verified; print dependencies and payment controls remain open.
- Original evidence: _permissions reported generic explicit grants, including for UAT accounts with combined operational grants. The source page uses many of those flags directly. FIN-GAP-027 bound validation to its real selector, and FIN-GAP-028 now scopes return; other controls still need equivalent review against their source current-office, maker-checker and pending-work rules. can_amend_nonfinancial does not yet exclude a pending amendment even though its service does.
...
- Next step: reproduce and repair signing-copy/packet dependency parity and remaining payment-form controls. Preserve reasoned replacement printing and existing source authority; complete before detailed DV personal-history expansion.
- Scope notes from artifact review: signing-copy creation also calls DV output generation, which requires the separate preparation grant and pinned owner; its queue currently checks only control_dv_printing. Packet assembly needs both custody grants and, when creating a new packet, TracePoint preparation authority. Keep permission-to-act separate from financial/document readiness, and preserve reasoned replacement printing after packet circulation. FIN-GAP-030 handles the legacy link/output controls separately.
- v0.7.58 verified partial repair: five page/HTTP tests reproduced 13 failed assertions in 16.500 seconds. UAT mutation flags, payable/DV preparation-review scope, ordered signature form choices, the source-filtered main-task banner and duplicate pending-amendment controls now reuse existing rules. Printing dependencies and remaining payment controls stay open; this does not close the whole finding.
- Payment follow-up from source inspection: CheckIssueForm's remaining-amount default excludes cancelled checks but still counts bank-returned checks; issue_check excludes both from its locked active total. CheckReleaseForm filters advised/acknowledged instruments but not the stale/returned operational blockers enforced by release_check. Reproduce these defaults/choices alongside current-office and terminal-state controls before repair; source amount and exception guards remain authoritative.
