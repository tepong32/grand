...
- Severity/status: **MEDIUM - OPEN**. The observed transaction rolls back and exactly one active match survives; duplicate financial records were not reproduced by this experiment.
...
- Next: provide controlled handling at the complete bank-match/automatic-match transaction boundary, preserve rollback and source revalidation, and enforce the intended active-match identities on native MySQL. Rerun the real concurrent HTTP case and affected bank reconciliation/correction/reporting flows. Do not broadly retry unrelated financial services or fabricate a duplicate-data cleanup.
