...
A different assigned witness can pass only when every required control is confirmed, no exception remains unresolved, and both actual targets are within the locally approved limits. Otherwise the witness requires a reasoned rerun. Passed evidence becomes immutable. The final cutover record must select the passed rehearsal from that exact cycle, so its backup ID and evidence checksum remain bound to the authority decision and schema-v8 portable evidence package.
