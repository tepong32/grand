...
ACCOUNTING_GUIDES = (
    ...
    },
    {
        ...
        "order": 60,
        "steps": (
            ("Find the reporting work", "From My Work, choose the exact generation, failed-rerun, control-blocked, independent-review, or approval action allowed to your role. The link opens the same filtered Report run register; report, control, format, period year, and plain search may narrow it further.", "The task, screen, and summary export share one Accounting visibility/action scope while each run keeps its detailed output, control evidence, and reproduction receipt.", "The task and summary register do not alter or approve a report and are not a signed statement, filed return, or permission to see another user's restricted runs.", "Open Reports", "reporting:workspace"),
            ...
            ("Prepare the statement notes", "Open Statement notes and select Position, Performance, Changes in Net Assets and Cash Flow for the same period. Complete each applicable topic from retained schedules or record a specific not-applicable reason. After independent review, authorized report downloaders can download the four original statement files and printable notes together.", "Submission checks matching source journals and financial totals, then freezes all four report references and disclosures. An issued bundle remains an exact historical copy.", "Candidate topics are prompts, not an automatic COA or local disclosure checklist. Confirm current applicability before locally accepted use.", "Open Statement notes", "reporting:statement_note_set_list"),
            ...
            ("Accept the actual local form", "Open Local form acceptance after template activation. Use only the preparation, returned-correction, test-witness, or acceptance-review queue shown for your current permission; each My Work count opens that same exact office-scoped list. When useful, choose Use DBM starter and create the matching editable candidate. Compare every candidate section with the current blank/redacted local form, record whether it matched or does not apply with a retained reference, then confirm the actual source, delivery, people, copies, deadlines, custody, paper, pagination, overflow, and accessibility behavior. Perform every practical test and send each attempt to a different witness.", "A different form reviewer accepts only the exact checksummed reference and activated template after every candidate row is locally resolved and all required categories have a current witnessed pass; the detailed acceptance packet stays separate from the summary register.", "The 31 DBM starters, queue counts, and summary rows are not installed official forms. Your own test or submitted form is excluded from the corresponding independent queue; failed attempts remain visible and accepted changes use a fully retested successor.", "Open Local form acceptance", "reporting:local_form_workspace"),
            ...
            ("Review and approve independently", "A different authorized reviewer checks the actual output SHA-256, dataset snapshot and row count, source count/control checksum, reproduction key, source entries, exact local template, and applicability record before review and approval.", "GRAND re-locks the current run and recomputes retained integrity evidence at both decisions; the approval is attributable and the prior run remains retained if superseded.", "The creator cannot review or approve the same run. Checksum drift, pilot layouts, control differences, and pending local applicability stay blocked; generate a successor instead of editing evidence.", "", ""),
    ...
    },
    {
        ...
        "order": 50,
        "steps": (
            ("Find the month that needs work", "Use status, fund, bank-account, statement-year, next-action, or search filters on the monthly register. Needs a statement, Staged controls need correction, Returned, and Validated isolate the preparer's safe work windows.", "Each row states the next evidence-based action and an authorized user can export the exact filtered multi-statement register.", "Queue/export evidence does not approve a BRS or replace the detailed statement review.", "Open Bank Reconciliation", "accounting:bank_reconciliation_workspace"),
            ...
            ("Export portable evidence", "Use Export these statements for a multi-month control register, and open one statement to export its row-by-row matches, timing items, controls, and checksums. GRAND archives the same bytes and manifest inside the department/user/category TraceSync-ready folder tree.", "The queue overview and detailed reconciliation evidence remain separately usable and portable.", "Neither CSV becomes a signed or locally accepted BRS by export alone.", "", ""),
        ),
    },
    {
        ...
        "order": 51,
        "steps": (
            ("Open the independent-review queue", "Filter What needs attention to Waiting for independent review, then narrow by fund, bank account, year, or statement reference. Export the filtered register when a portable review schedule is needed.", "Only submitted monthly reconciliations in the acting Accounting department remain visible, with checksums and next actions.", "The summary register is not the reviewed bank statement, detailed match evidence, or signed BRS.", "Open Bank Reconciliation", "accounting:bank_reconciliation_workspace"),
    ...
    },
    {
        ...
        "order": 80,
        "steps": (
            ("Find the exact Accounting or custody work", "Start from My Work when you need the exact DV preparation, signing-copy, print-recording, packet-assembly, or earliest wet-signature-return action. Read its office, state, evidence revision, and exception, then open the shared case. Use Finance Queue filters for the broader stage/custody register and Export DV custody history when authorized.", "Each action stays separate and points to the current shared case; controlled print versions and superseded evidence remain visible.", "A task or custody register is not a wet signature, approval, payment authority, or proof that the starter/pilot form is locally accepted. Stop on any unexplained amount difference.", "Review My Work", "finance_operations:my_work"),
            ("Open your Accounting case", "Select a case at Accounting preparation and review its claim-to-allocation control, relationship types, recognition/adjustment decision, Budget classification, claimant/payee, and documents.", "The shared case—not a copied transaction—is open with zero relationship difference.", "Current Voucher Workbench controls remain a controlled UAT slice until parent roadmap gates pass.", "", ""),
    ...
    },
    {
        ...
        "order": 85,
        "steps": (
            ("Open your exact validation item", "From Work needing attention, open the signed voucher assigned to your Accounting office. Read its Task ID, evidence revision, recognition route, and any amount, allocation, packet, or posting-rule exception before acting.", "The exact item and Voucher queue apply the same office, UAT, and DV-preparer separation rules.", "Stop on even a one-cent unexplained difference. The task does not validate, post, or assign the voucher.", "Review My Work", "finance_operations:my_work"),
    ...
    },
    {
        ...
        "order": 10,
        "steps": (
            ("Find the exact setup action", "From Work needing attention, use preparation, independent review, future scheduling, or activation only when that named action matches your role. Read the stable Task ID, exact effectivity target when applicable, and source version before opening the release.", "The exact item and source register use the same office, lifecycle, effectivity, permission, and maker-checker rules.", "A target is the retained effective date, not an invented approval deadline. A task does not approve, schedule, activate, or make a starter locally accepted.", "Open Finance Setup", "finance:workspace"),
...
REQUESTING_GUIDES = (
    ...
    },
    {
        ...
        "steps": (
            ...
            ("Export the right level safely", "Use Export these cases for the current one-row-per-case queue register, or Export transaction inside one case for relationship, decision, and documentary detail. GRAND retains the same spreadsheet-safe CSV bytes under department/user/category/year/month with a sibling checksum manifest.", "The visible queue and detailed evidence can be copied through the single TraceSync-ready root without mixing their purposes.", "Neither export is approval, payment authority, or automatically an official COA/DBM/local form.", "", ""),
        ),
    },
)
...
BUDGET_GUIDES = (
    {
        ...
        "order": 60,
        "steps": (
            ("Find the reporting work", "From My Work, choose the exact generation, failed-rerun, control-blocked, independent-review, or approval action allowed to your role. The link opens the same Budget report rows and may be narrowed by report, control, format, period year, or search.", "The task, screen, and summary export share one role-shaped scope while every run retains its output, source controls, and reproduction receipt.", "The task and summary register do not alter or approve a run and are not an accepted DBM/COA schedule, signed form, or permission to see another user's restricted reports.", "Open Reports", "reporting:workspace"),
            ...
            ("Record local form acceptance", "Open Local form acceptance and use the exact preparation, returned-correction, test-witness, or acceptance-review queue shown for your permission; My Work opens the same office-scoped list. Choose Use DBM starter when the office is comparing one of the 31 listed LBP/LBA/LBR/LBE/LBAc forms. Create only the matching editable candidate, compare each field/source/control/owner/print section with the current blank/redacted local form, record a retained local reference for every matched or not-applicable decision, then confirm the source, delivery, people, copies, deadlines, custody, and layout. Link only the activated report template and retain independently witnessed tests.", "The accepted form version pins the exact template, reference, locally resolved sections, practical results, people, and SHA-256 evidence while prior versions remain traceable; the detailed acceptance packet stays separate from the summary register.", "A DBM candidate, queue count, summary row, technically balanced Budget report, or public blank form does not establish local acceptance. Your own test/submission is not independent review work; repeat testing through a successor whenever the accepted form changes.", "Open Local form acceptance", "reporting:local_form_workspace"),
    ...
    },
    {
        ...
        "order": 2,
        "steps": (
            ("Review the Budget queue", "Open Obligation control and use fiscal-year, request-type, form, status, or next-action filters to find requests awaiting certification. Confirm the requesting office, form type, request reference, claimant/payee, particulars, date, and evidence reference.", "The reviewer is looking at the same submitted request, not a copied voucher record, and each row states its next action.", "The requesting-office submitter cannot certify the same request.", "Open Obligation Control", "budget:obligation_workspace"),
            ...
            ("Export and reconcile", "Export the visible control register to preserve the exact filtered queue and role scope. For detailed reconciliation, an authorized Budget user exports the full certified RAAO-equivalent movement registry. Preserve the entire TraceSync-ready export tree and sibling manifests.", "Both exports retain the downloaded bytes, SHA-256 evidence, and actor audit event; queue oversight remains visibly separate from certified movement detail.", "Neither controlled export establishes exact official-template acceptance until the LGU confirms it locally.", "Open Obligation Control", "budget:obligation_workspace"),
        ),
    },
    {
        ...
        "order": 3,
        "steps": (
            ("Choose operational authority", "Open Allotment control. Use fiscal-year, order-type, status, or next-action filters to find the current work, then choose the exact authorized annual, supplemental, or reenacted appropriation. Review authorized, released, held, unreleased, and executable totals.", "Each order states its next action and stays tied to one immutable appropriation schedule and fiscal year.", "An approved proposal is not enough; only operationally authorized appropriation is eligible.", "Open Allotment Control", "budget:allotment_workspace"),
            ...
            ("Correct and export without overwriting", "For a posted error or later action, create a linked adjustment, return, or cancellation order and explain the authority. Export the visible filtered control register for queue oversight; open a posted order to export its detailed movement schedule for reconciliation.", "The original and every successor remain reconstructible, while both downloaded exports and their checksums stay in the TraceSync-ready archive.", "Never edit or delete a posted order or movement, and do not treat a controlled export as proof of local form acceptance.", "Open Allotment Control", "budget:allotment_workspace"),
        ),
    },
    {
...
TREASURY_GUIDES = (
    {
        ...
        "order": 60,
        "steps": (
            ("Find the reporting work", "From My Work, choose the exact generation, failed-rerun, control-blocked, independent-review, or approval action allowed to your role. The link opens the same Treasury report rows and may be narrowed by report, control, format, period year, or search.", "The task, screen, and summary export share one role-shaped scope while every run retains its output, transaction controls, and reproduction receipt.", "The task and summary register do not alter or approve a run and are not a disbursement register, signed Treasury form, bank acknowledgement, or permission to see another user's restricted reports.", "Open Reports", "reporting:workspace"),
            ...
            ("Witness the actual Treasury form", "Use the Local form acceptance preparation, returned-correction, test-witness, or acceptance-review queue shown for your permission; My Work opens that same exact Treasury-office list. Link the activated register or Finance workbook and retain the blank/redacted reference. For an applicable LBP/LBA/LBR/LBE/LBAc form, an authorized preparer may begin with Use DBM starter, but must compare and locally resolve every candidate section before recording the actual signatory, copy, recipient, custody, deadline, paper, printer, overflow, and continuation rules. Perform or independently witness the applicable practical tests.", "Only a different witness decides a submitted test, and only a different reviewer can accept the exact checksummed form after every starter row is resolved and every current test passes; the detailed acceptance packet stays separate from the summary register.", "A DBM starter, queue count, or summary row is not automatically the Treasury's accepted form. Do not mark printer/form-stock testing not applicable when Treasury prints, signs, releases, or files paper; accepted changes use a successor and a fresh test cycle.", "Open Local form acceptance", "reporting:local_form_workspace"),
    ...
    },
    {
        ...
        "order": 5,
        "steps": (
            ("Open the exact cash-control item", "From My Work, open the policy or cash-position version in your permitted queue. Read the exact bank/fund, evidence version, source checksum, arithmetic, and any stop exception before acting.", "Preparation stays inside the owning Treasury office, while independent review may cover permitted Finance offices and excludes the maker.", "The card does not prepare, submit, approve, reserve, or release cash.", "Review My Work", "finance_operations:my_work"),
            ...
            ("Use the modification allowance", "Edit only a draft policy or position. Once submitted, returned, active, or approved, prepare a reasoned successor version; the old evidence is locked and leaves the preparation queue when its successor exists. Before any check exists, use the voucher's guided correction route for eligible DV fields and evidence.", "Prior cash and voucher evidence stays visible; issued instruments are never silently edited.", "After check issue, use cancellation/replacement or Accounting adjustment routes as applicable.", "", ""),
    ...
    },
    {
        ...
        "order": 10,
        "steps": (
            ("Open the exact remittance item", "From My Work, open the batch in your exact preparation, returned-correction, independent-review, or release queue. Confirm the owning Treasury office, fund, bank, agency, active-line total, control total, and source checksum.", "The source register and My Work use the same office, permission, UAT, state, and maker-checker filters.", "Cross-office Accounting review does not grant cross-office preparation, tax-evidence editing, or release authority.", "Review My Work", "finance_operations:my_work"),
    ...
    },
    {
        ...
        "order": 20,
        "steps": (
            ("Open the exact Treasury item", "From My Work, open the check-preparation case or the individual advised check assigned to your current Treasury office. Read the remaining net, bank/fund scope, source version, and any stop exception before acting.", "Check preparation stays case-based while release is one task per physical instrument.", "A work item does not issue or release money, and another office's case must not appear in your action queue.", "Review My Work", "finance_operations:my_work"),
            ...
            ("Register the physical check", "Record the controlled check number, amount to cent precision, active bank account, fund, and issue evidence. Reconcile the running active-instrument total to the exact DV net; issued numbers are never silently reused.", "The case carries a traceable payment instrument and the remaining net updates without changing the task identity.", "Stop on any cent difference, over-issue, inactive bank setup, or mixed-bank advice batch. Issuance closes the convenience modification window for voucher fields.", "", ""),
            ...
            ("Release only after advice", "For the exact instrument task, confirm the current advice is bank-acknowledged, the check has no stale/returned block, and the claimant authorization is effective today. Record the actual receipt reference at handover. If the pinned payment rule posts on release, the case moves temporarily to Accounting.", "The exact released instrument, claimant, receipt, amount, and bank acknowledgement become an immutable payment-event handoff.", "An approval or bank submission is not an acknowledgement. Never release to an expired/unconfigured claimant or without the actual receipt reference.", "", ""),
...
REQUESTING_GUIDES = REQUESTING_GUIDES + (
    {
        ...
        "steps": (
            ...
            ("Read an exact work item when available", "For Finance Setup, discovery decisions, Budget versions/allotments/obligations, payable preparation/review, DV preparation/controlled custody/wet-signature return, Accounting validation/JEV preparation/posting, Treasury check preparation/individual release, remittance, cash policy/position, report generation/reconciliation/review/approval, Field-operation cycle and nested actions, and Local forms, read the stable Task ID, source reference, exact action and gate, permitted queue, scope, timing basis, source version, and exception before opening the record.", "The same source and action keep the same Task ID while applicable, while separate actions on one source remain distinguishable; Reporting tasks recheck retained dataset, source/control, output, and reproduction checksums.", "The item does not copy, assign, approve, validate, sign, print, issue, release, remit, reserve, post, generate, or complete the source record. Every applicable financial and report-control difference must reconcile to zero before the governed next step.", "Review My Work", "finance_operations:my_work"),
            ...
            ("Export and safeguard when authorized", "Use the workspace export for the exact visible scope. Preserve the complete configured GRAND export root so department, user, category, year, month, file, and checksum manifest stay together for TraceSync or another approved copy method.", "Portable evidence keeps its lineage and integrity details.", "An export is not automatically an official form, statutory submission, database backup, or authority decision.", "", ""),
            ("Resume this guide privately", "Use the floating ? while keeping the current page open. Check off steps only when helpful to you.", "Your own browser can resume the guide where you stopped.", "Tutorial progress is private learning state—not attendance, competence, UAT acceptance, approval, or employee-evaluation evidence.", "", ""),
        ),
    },
    {
...
ACCOUNTING_GUIDES = ACCOUNTING_GUIDES + (
    {
        ...
        "steps": (
            ...
            ("Work the items needing attention", "From My Work, use Draft or returned decisions I may prepare for rows you own or may manage, and Submitted decisions assigned to me for review only for independent decisions naming you. Read the stable Task ID, affected scope, retained review target, source version, and safe blocker or returned notice before opening the decision. In the source register, use the same filter; blocker, all-awaiting-review, overdue, and all-returned choices remain oversight views.", "The exact item, source list, and manager's filtered department export apply the same scope and role rules; an unresolved blocker is stated only against its named affected scope.", "A retained review target is not an invented statutory deadline. An oversight row is not automatically your assignment, and a task does not accept, return, or clear evidence.", "Open Decisions & Evidence", "finance:discovery_workspace"),
    ...
    },
    {
        ...
        "order": 89,
        "steps": (
            ("Triage the exact field action", "From Work needing attention, choose source locking, cycle preparation/execution, independent reconciliation, defect correction/review, exercise completion/witnessing, stakeholder decision, or cutover authority only when that named action matches your role. The source register uses the same filter; reconciled, authorized, and returned choices remain oversight views.", "The attention group still counts cycles for source-register parity. The exact-item table gives every actionable defect, exercise, stakeholder decision, and cutover record its own stable identity and source link; named cross-office users see only their assigned records.", "Do not compare the cycle-group count with the nested-record total as if they measured the same thing. Neither accepts a checkpoint, replaces detailed evidence, nor authorizes GRAND. Private tutorial checkmarks remain personal resume aids.", "Open Shadow operation", "finance:shadow_workspace"),
...
FINANCE_MY_WORK_GUIDES = {
    "accounting": {
        ...
        "steps": (
            ...
            ("Read an exact work item", "For Finance Setup, discovery decisions, payable review, DV preparation, controlled signing-copy/print/packet custody, earliest wet-signature return, Accounting validation, JEV preparation/posting, opening-balance staging/submission/review/posting/reconciliation, Field-operation cycle and nested actions, and Local Forms, read the stable Task ID, controlled type, source reference, action, gate, permitted queue, target when retained, timing basis, source version, and exception before opening the record.", "The same source and action keep the same Task ID while applicable. Accounting validation and posting use current-office and maker-checker boundaries; print/custody steps do not collapse into one action.", "The item is a read-only pointer. Opening tasks retain source/row checksums and independent review/posting; changed evidence requires governed return and revalidation. Opening dates are not deadlines. Stop on any one-cent DV, obligation, allocation, or debit-credit difference and on a closed period; a screen receipt is not the physical signature or a posting action.", "Review My Work", "finance_operations:my_work"),
    ...
    "treasury": {
        ...
        "steps": (
            ...
            ("Choose the exact kind of work", "Use the shared voucher action row for ordinary checks or release; the returned-payment rows for Treasury clarification or an Accounting-cleared replacement; the bank-advice or remittance row for its named state; or the separate cash-policy and cash-position rows.", "A returned bank item is not replacement-ready until Accounting completes its recorded decision and any required posting.", "Do not infer bank acknowledgement, cash availability, replacement authority, release authority, or payment from a count.", "", ""),
            ("Open the exact queue", "Choose Open exact queue and confirm the filtered records before acting.", "The Voucher Workbench, returned-item, bank-advice, remittance, or cash-control register shows the same role-scoped state and record type.", "The source record remains authoritative and retains maker-checker, clarification-successor, reversal, replacement, and correction controls.", "", ""),
