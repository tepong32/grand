...
BUDGET_GUIDES = (
    {
        ...
        "steps": (
            ...
            ("Record local form acceptance", "Open Local form acceptance and use status, GRAND source, delivery, next-action, or search filters to find the blocking Budget form; export the visible register only as portable oversight. Choose Use DBM starter when the office is comparing one of the 31 listed LBP/LBA/LBR/LBE/LBAc forms. Create only the matching editable candidate, compare each field/source/control/owner/print section with the current blank/redacted local form, record a retained local reference for every matched or not-applicable decision, then confirm the source, delivery, people, copies, deadlines, custody, and layout. Link only the activated report template and retain independently witnessed tests.", "The accepted form version pins the exact template, reference, locally resolved sections, practical results, people, and SHA-256 evidence while prior versions remain traceable; the detailed acceptance packet stays separate from the summary register.", "A DBM candidate, summary row, technically balanced Budget report, or public blank form does not establish local acceptance. Unconfirmed rows block submission; repeat testing through a successor whenever the accepted form changes.", "Open Local form acceptance", "reporting:local_form_workspace"),
