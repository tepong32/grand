...
REQUESTING_GUIDES = REQUESTING_GUIDES + (
    {
        ...
        "steps": (
            ...
            ("Read an exact work item when available", "For Finance Setup, discovery decisions, Budget versions/allotments/obligations, payable preparation/review, DV preparation/controlled custody/wet-signature return, Field-operation cycle and nested actions, and Local forms, read the stable Task ID, source reference, exact action and gate, permitted queue, scope, timing basis, source version, and exception before opening the record.", "The same source and action keep the same Task ID while applicable, while separate actions on one source remain distinguishable.", "The item does not copy, assign, approve, sign, print, or complete the source record. Budget, payable, and DV control differences must reconcile to zero before the governed next step.", "Review My Work", "finance_operations:my_work"),
...
FINANCE_MY_WORK_GUIDES = {
    "accounting": {
        ...
        "steps": (
            ...
            ("Read an exact work item", "For Finance Setup, discovery decisions, payable review, DV preparation, controlled signing-copy/print/packet custody, earliest wet-signature return, Field-operation cycle and nested actions, and Local Forms, read the stable Task ID, controlled type, source reference, action, gate, permitted queue, target when retained, timing basis, source version, and exception before opening the record.", "The same source and action keep the same Task ID while applicable. Payable and DV work use the current-office and maker-checker boundaries; print/custody steps do not collapse into one action.", "The item is a read-only pointer. Stop on any unexplained gross-deduction-net or certified-obligation difference; a screen receipt is not the physical signature.", "Review My Work", "finance_operations:my_work"),
