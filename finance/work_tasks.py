...
def _nested_field_task(item, action_key, spec, department, today):
    ...
    if action_key in FIELD_CONTROL_ACTION_SPECS:
        ...
        return field_control_task(item, action_key, spec, today)
    from .field_plan_register import FIELD_PLAN_ACTION_SPECS
    if action_key in FIELD_PLAN_ACTION_SPECS:
        from .work_field_plans import field_plan_task
        return field_plan_task(item, action_key, spec, today)
    source_kind = {
...
def finance_work_tasks(user, *, display_limit=100, view="ready", planned_days=7):
    ...
    if view == "completed":
        from .work_completed import completed_report_tasks, completed_cash_tasks, completed_accounting_tasks, completed_budget_tasks, completed_payment_handoff_tasks, completed_setup_tasks, completed_discovery_tasks, completed_payable_tasks, completed_dv_tasks, completed_returned_payment_tasks, completed_instrument_tasks
        tasks = completed_payable_tasks(user, department, today)
        tasks.extend(completed_dv_tasks(user, department, today))
        from .work_voucher_posting_history import completed_voucher_posting_tasks
        tasks.extend(completed_voucher_posting_tasks(user, department, today))
        tasks.extend(completed_returned_payment_tasks(user, department, today))
        ...
        from .work_field_sources import completed_field_source_tasks
        tasks.extend(completed_field_source_tasks(user, department, today))
        ...
        tasks.extend(completed_payment_handoff_tasks(user, department, today))
    elif view == "waiting":
    ...
    return {
        ...
        "task_coverage": ("Attributed source staging and independent layout decisions", "Attributed scheduled-run and qualification-evidence actions", "Attributed field-plan submission and independent decisions", "Attributed local-form tests and acceptance decisions", "Attributed stakeholder and cutover decision events", "Attributed field-cycle, exercise scheduling/results and defect registration/escalation/decision events", "Attributed accountability profile/package submission and decision events", "Attributed manual report generation and report decision events", "Attributed cash policy and position submission/decision events", "Attributed bank-reconciliation submission and decision events", "Attributed instrument issue, advice handoff, release and cancellation events", "Attributed returned-payment review-version events", "Attributed DV preparation, print/packet, amendment, signature-return and validation events", "Attributed voucher posted-source synchronization events", "Attributed payable review handoffs", "Attributed discovery decision events", "Attributed setup release transitions", "Attributed bank-advice and remittance handoff events", "Attributed Budget call, proposal, appropriation, allotment and obligation events", "Attributed JEV submission/posting/return events", "Opening-balance submission and decision events", "Period-close and reopen decision events") if view == "completed" else ("Own staged sources awaiting independent layout review", "Own submitted scheduled runs and qualification evidence", "Own submitted reconciliation, readiness and qualification plans", "Own local-form test and acceptance submissions", "Own prepared stakeholder and submitted cutover decision records", "Own submitted field cycles, scheduled exercises and registered/submitted defect handoffs", "Own submitted accountability profiles and packages", "Own generated reports awaiting review or approval", "Submitted cash policies and positions", "Payable, DV, Accounting posting, Treasury, bank-advice and event-posting handoffs", "Submitted bank reconciliations", "Returned-payment review, clarification, posting and replacement handoffs", "Submitted discovery decisions", "Submitted Finance setup releases", "Budget proposals, allotment orders and obligation requests", "Personal submitted JEVs", "Opening balances", "Submitted period-close checklists and own period-reopen requests", "Bank advice", "Remittance review, release and Accounting posting") if view == "waiting" else (
