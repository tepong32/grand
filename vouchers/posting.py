...
def materialize_voucher_journal(posting_request, actor):
    if posting_request.payload.get('advance_recognition_correction'):
        from .advance_recognition_corrections import materialize
        return materialize(posting_request, actor)
    if posting_request.payload.get('advance_application_correction'):
        from .liquidation_corrections import materialize
        return materialize(posting_request, actor)
    ...
    if (posting_request.payload.get("earlier_accrual") or posting_request.payload.get("deduction_correction")
            or posting_request.payload.get('advance_payment_cancellation') or posting_request.payload.get('advance_payment_return')):
...
def _materialize_voucher_journal(posting_request, actor):
    ...
    try:
        ...
        with transaction.atomic(using="finance"):
            ...
            claim_amounts = None
            reversal_of = None
            if payload.get('advance_payment_cancellation') or payload.get('advance_payment_return'):
            ...
            entry = JournalEntry(
                ...
                source_snapshot={
                    ...
                    **({'advance_payment_return':payload['advance_payment_return']} if payload.get('advance_payment_return') else {}),
                },
...
def reconcile_posted_voucher_entry(entry, actor):
    ...
    if entry.source_snapshot.get('advance_recognition_correction'):
        from .advance_recognition_corrections import reconcile
        return reconcile(entry, actor)
    if entry.source_snapshot.get('advance_application_correction'):
        from .liquidation_corrections import reconcile
        return reconcile(entry, actor)
